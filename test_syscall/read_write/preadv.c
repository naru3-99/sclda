...
int main() {
    ...
    int fd = open("example_preadv.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd == -1) {
        perror("open");
        exit(EXIT_FAILURE);
    }
    ...
    // ファイルを読み込み用にオープン
    fd = open("example_preadv.txt", O_RDONLY);
    if (fd == -1) {
        perror("open");
        exit(EXIT_FAILURE);
    }
    ...
    // preadvを呼び出し、ファイルのオフセット10からデータを読み込む
    off_t offset = 10;
    ssize_t nread = preadv(fd, iov, 3, offset);
    if (nread == -1) {
        perror("preadv");
        close(fd);
        exit(EXIT_FAILURE);
