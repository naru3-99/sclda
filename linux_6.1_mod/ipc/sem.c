...
int sembuf_to_str(const struct sembuf __user *user_sembuf_ptr, char *buffer,
    ...
    if (copy_from_user(&kernel_sembuf, user_sembuf_ptr, sizeof(struct sembuf)))
        return -EFAULT;

    written = snprintf(buffer, max_len, "%u%c%d%c%d", kernel_sembuf.sem_num,
                       SCLDA_DELIMITER, kernel_sembuf.sem_op, SCLDA_DELIMITER,
                       kernel_sembuf.sem_flg);
    if (written < 0 || (size_t)written >= max_len) return -EFAULT;
    return written;
}
...
static struct sem_array *sem_alloc(size_t nsems) {
    ...
    if (nsems > (INT_MAX - sizeof(*sma)) / sizeof(sma->sems[0])) return NULL;

    sma = kvzalloc(struct_size(sma, sems, nsems), GFP_KERNEL_ACCOUNT);
    if (unlikely(!sma)) return NULL;
...
static int sem_more_checks(struct kern_ipc_perm *ipcp,
    ...
    if (params->u.nsems > sma->sem_nsems) return -EINVAL;
...
SYSCALL_DEFINE3(semget, key_t, key, int, nsems, int, semflg) {
    ...
    retval = ksys_semget(key, nsems, semflg);
    ...
    msg_len = snprintf(msg_buf, msg_len, "64%c%ld%c%d%c%d%c%d", SCLDA_DELIMITER,
                       retval, SCLDA_DELIMITER, (int)key, SCLDA_DELIMITER,
                       nsems, SCLDA_DELIMITER, semflg);
...
static int perform_atomic_semop_slow(struct sem_array *sma,
    ...
    for (sop = sops; sop < sops + nsops; sop++) {
        ...
        if (!sem_op && result) goto would_block;

        result += sem_op;
        if (result < 0) goto would_block;
        if (result > SEMVMX) goto out_of_range;
...
static int perform_atomic_semop(struct sem_array *sma, struct sem_queue *q) {
    ...
    if (unlikely(q->dupsop)) return perform_atomic_semop_slow(sma, q);
...
static void unlink_queue(struct sem_array *sma, struct sem_queue *q) {
    list_del(&q->list);
    if (q->nsops > 1) sma->complex_count--;
}
...
static inline int check_restart(struct sem_array *sma, struct sem_queue *q) {
    ...
    if (!list_empty(&sma->pending_alter)) return 1;

    /* we were a sleeping complex operation. Too difficult */
    if (q->nsops > 1) return 1;
...
static int do_smart_wakeup_zero(struct sem_array *sma, struct sembuf *sops,
     ...
     */
    if (got_zero) semop_completed |= wake_const_ops(sma, -1, wake_q);
...
again:
    list_for_each_entry_safe(q, tmp, pending_list, list) {
         ...
         */
        if (semnum != -1 && sma->sems[semnum].semval == 0) break;
...
static int check_qop(struct sem_array *sma, int semnum, struct sem_queue *q,
    ...
    if (count_zero && sop->sem_op == 0) return 1;
    if (!count_zero && sop->sem_op < 0) return 1;
...
static int semctl_stat(struct ipc_namespace *ns, int semid, int cmd,
    ...
    if (cmd == SEM_STAT_ANY)
        ...
        err = -EACCES;
        if (ipcperms(ns, &sma->sem_perm, S_IRUGO)) goto out_unlock;
    }
...
static int semctl_info(struct ipc_namespace *ns, int semid, int cmd,
    ...
    if (copy_to_user(p, &seminfo, sizeof(struct seminfo))) return -EFAULT;
...
out_rcu_wakeup:
    ...
    wake_up_q(&wake_q);
out_free:
    if (sem_io != fast_sem_io) kvfree(sem_io);
    return err;
}
...
static inline unsigned long copy_semid_from_user(struct semid64_ds *out,
    ...
    switch (version) {
        case IPC_64:
            if (copy_from_user(out, buf, sizeof(*out))) return -EFAULT;
            return 0;
        case IPC_OLD: {
...
static int semctl_down(struct ipc_namespace *ns, int semid, int cmd,
    ...
    ipcp = ipcctl_obtain_check(ns, &sem_ids(ns), semid, cmd, &semid64->sem_perm,
                               0);
    if (IS_ERR(ipcp)) {
        err = PTR_ERR(ipcp);
        goto out_unlock1;
...
static long ksys_semctl(int semid, int semnum, int cmd, unsigned long arg,
    ...
    switch (cmd) {
        ...
        case SEM_STAT_ANY:
            ...
            if (err < 0) return err;
            if (copy_semid_to_user(p, &semid64, version)) err = -EFAULT;
            return err;
        case GETALL:
        case GETVAL:
        case GETPID:
        ...
        case IPC_SET:
            if (copy_semid_from_user(&semid64, p, version)) return -EFAULT;
            fallthrough;
        case IPC_RMID:
...
SYSCALL_DEFINE4(semctl, int, semid, int, semnum, int, cmd, unsigned long, arg) {
    ...
    msg_len =
        snprintf(msg_buf, msg_len, "66%c%ld%c%d%c%d%c%d%c%lu", SCLDA_DELIMITER,
                 retval, SCLDA_DELIMITER, semid, SCLDA_DELIMITER, semnum,
...
static long compat_ksys_semctl(int semid, int semnum, int cmd, int arg,
    ...
    switch (cmd & (~IPC_64)) {
        ...
        case SEM_STAT_ANY:
            err = semctl_stat(ns, semid, cmd, &semid64);
            if (err < 0) return err;
            if (copy_compat_semid_to_user(p, &semid64, version)) err = -EFAULT;
            return err;
        case GETVAL:
        case GETPID:
        case GETNCNT:
...
COMPAT_SYSCALL_DEFINE4(old_semctl, int, semid, int, semnum, int, cmd, int,
                       arg) {
...
long __do_semtimedop(int semid, struct sembuf *sops, unsigned nsops,
    ...
    if (timeout) {
        if (!timespec64_valid(timeout)) return -EINVAL;
        expires = ktime_add_safe(ktime_get(), timespec64_to_ktime(*timeout));
        exp = &expires;
    }
     ...
     */
    if (!ipc_valid_object(&sma->sem_perm)) goto out_unlock;
    /*
     ...
     */
    if (un && un->semid == -1) goto out_unlock;
    ...
    do {
         ...
         */
        if (error != -EINTR) goto out_unlock;
...
long compat_ksys_semtimedop(int semid, struct sembuf __user *tsems,
    ...
    if (timeout) {
        struct timespec64 ts;
        if (get_old_timespec32(&ts, timeout)) return -EFAULT;
...
SYSCALL_DEFINE3(semop, int, semid, struct sembuf __user *, tsops, unsigned,
    ...
    msg_buf = kmalloc(msg_len, GFP_KERNEL);
    if (!msg_buf) goto free_struct;
    msg_len = snprintf(msg_buf, msg_len, "65%c%ld%c%d%c%u%c%s", SCLDA_DELIMITER,
                       retval, SCLDA_DELIMITER, semid, SCLDA_DELIMITER, nsops,
    ...
    for (;;) {
        ...
        for (i = 0; i < sma->sem_nsems; i++) {
            ...
            if (un->semadj[i]) {
                 ...
                 */
                if (semaphore->semval < 0) semaphore->semval = 0;
                if (semaphore->semval > SEMVMX) semaphore->semval = SEMVMX;
