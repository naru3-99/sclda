...
SYSCALL_DEFINE2(msgget, key_t, key, int, msgflg)
{
	...
	msg_len = snprintf(msg_buf, msg_len, "68%c%ld%c%d%c%d", SCLDA_DELIMITER,
		       ...
		       struct msginfo *msginfo)
{
	...
	if (cmd == MSG_INFO) {
		msginfo->msgmap = min_t(
			int, percpu_counter_sum(&ns->percpu_msg_hdrs), INT_MAX);
		msginfo->msgtql =
			min_t(int, percpu_counter_sum(&ns->percpu_msg_bytes),
			      INT_MAX);
	} else {
	  ...
	  long (*msg_handler)(void __user *, struct msg_msg *, size_t))
{
	...
	for (;;) {
		...
		if (!IS_ERR(msg)) {
			...
			percpu_counter_sub_local(&ns->percpu_msg_bytes,
						 msg->m_ts);
			percpu_counter_sub_local(&ns->percpu_msg_hdrs, 1);
			ss_wakeup(msq, &wake_q, false);
