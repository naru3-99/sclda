...
static int madvise_update_vma(struct vm_area_struct *vma,
    ...
    *prev =
        vma_merge(mm, *prev, start, end, new_flags, vma->anon_vma, vma->vm_file,
                  pgoff, vma_policy(vma), vma->vm_userfaultfd_ctx, anon_name);
    if (*prev) {
        vma = *prev;
        goto success;
    }
    ...
    *prev = vma;
    ...
    if (start != vma->vm_start) {
        if (unlikely(mm->map_count >= sysctl_max_map_count)) return -ENOMEM;
...
static int swapin_walk_pmd_entry(pmd_t *pmd, unsigned long start,
    ...
    if (pmd_none_or_trans_huge_or_clear_bad(pmd)) return 0;
    ...
    for (index = start; index != end; index += PAGE_SIZE) {
        ...
        if (!is_swap_pte(pte)) continue;
        entry = pte_to_swp_entry(pte);
        if (unlikely(non_swap_entry(entry))) continue;

        page = read_swap_cache_async(entry, GFP_HIGHUSER_MOVABLE, vma, index,
                                     false, &splug);
...
static void force_shm_swapin_readahead(struct vm_area_struct *vma,
    ...
    xas_for_each(&xas, page, end_index) {
        ...
        if (!xa_is_value(page)) continue;
        swap = radix_to_swp_entry(page);
        /* There might be swapin error entries in shmem mapping. */
        if (non_swap_entry(swap)) continue;
        xas_pause(&xas);
        rcu_read_unlock();

        page = read_swap_cache_async(swap, GFP_HIGHUSER_MOVABLE, NULL, 0, false,
                                     &splug);
        if (page) put_page(page);
...
static long madvise_willneed(struct vm_area_struct *vma,
    ...
    *prev = NULL; /* tell sys_madvise we drop mmap_lock */
    get_file(file);
    offset =
        (loff_t)(start - vma->vm_start) + ((loff_t)vma->vm_pgoff << PAGE_SHIFT);
    mmap_read_unlock(mm);
...
static int madvise_cold_or_pageout_pte_range(pmd_t *pmd, unsigned long addr,
    ...
    if (pmd_trans_huge(*pmd)) {
        ...
        if (page_mapcount(page) != 1) goto huge_unlock;
    ...
    huge_unlock:
        spin_unlock(ptl);
        if (pageout) reclaim_pages(&page_list);
        return 0;
    }
...
regular_page:
    ...
    for (; addr < end; pte++, addr += PAGE_SIZE) {
        ...
        if (!page || is_zone_device_page(page)) continue;
         ...
         * non-LRU page.
         */
        if (!PageLRU(page) || page_mapcount(page) != 1) continue;
...
static long madvise_cold(struct vm_area_struct *vma,
    ...
    *prev = vma;
    if (!can_madv_lru_vma(vma)) return -EINVAL;
...
static long madvise_pageout(struct vm_area_struct *vma,
    ...
    *prev = vma;
    ...
    if (!can_do_pageout(vma)) return 0;
...

{
    ...
    if (pmd_trans_huge(*pmd))
        if (madvise_free_huge_pmd(tlb, vma, pmd, addr, next)) goto next;
    ...
    for (; addr != end; pte++, addr += PAGE_SIZE) {
        ...
        if (folio_test_large(folio)) {
            if (folio_estimated_sharers(folio) != 1) break;
            folio_get(folio);
...
static int madvise_free_single_vma(struct vm_area_struct *vma,
    ...
    if (!vma_is_anonymous(vma)) return -EINVAL;

    range.start = max(vma->vm_start, start_addr);
    if (range.start >= vma->vm_end) return -EINVAL;
    range.end = min(vma->vm_end, end_addr);
    if (range.end <= vma->vm_start) return -EINVAL;
    mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, mm, range.start,
                            range.end);
...
static bool madvise_dontneed_free_valid_vma(struct vm_area_struct *vma,
    ...
    if (behavior != MADV_DONTNEED && behavior != MADV_DONTNEED_LOCKED)
        return false;
    if (start & ~huge_page_mask(hstate_vma(vma))) return false;
...
static long madvise_populate(struct vm_area_struct *vma,
    ...
    *prev = vma;
    ...
    while (start < end) {
        ...
        pages = faultin_vma_page_range(vma, start, tmp_end, write, &locked);
        if (!locked) {
            mmap_read_lock(mm);
            locked = 1;
            *prev = NULL;
...
static long madvise_remove(struct vm_area_struct *vma,
    ...
    *prev = NULL; /* tell sys_madvise we drop mmap_lock */
    ...
    error = vfs_fallocate(f, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, offset,
                          end - start);
    fput(f);
    mmap_read_lock(mm);
    return error;
...
static int madvise_vma_behavior(struct vm_area_struct *vma,
    ...
    switch (behavior) {
        ...
        case MADV_DONTFORK:
            ...
            break;
        case MADV_DOFORK:
            if (vma->vm_flags & VM_IO) return -EINVAL;
            new_flags &= ~VM_DONTCOPY;
            break;
        case MADV_WIPEONFORK:
            /* MADV_WIPEONFORK is only supported on anonymous memory. */
            if (vma->vm_file || vma->vm_flags & VM_SHARED) return -EINVAL;
...
static int madvise_inject_error(int behavior, unsigned long start,
    ...
    for (; start < end; start += size) {
        ...
        if (behavior == MADV_SOFT_OFFLINE) {
            ...
            pr_info(
                "Injecting memory failure for pfn %#lx at process virtual "
                "address %#lx\n",
                pfn, start);
 ...
 */
static int madvise_walk_vmas(
    struct mm_struct *mm, unsigned long start, unsigned long end,
    unsigned long arg,
    int (*visit)(struct vm_area_struct *vma, struct vm_area_struct **prev,
                 unsigned long start, unsigned long end, unsigned long arg)) {
    struct vm_area_struct *vma;
    struct vm_area_struct *prev;
    unsigned long tmp;
...
SYSCALL_DEFINE5(process_madvise, int, pidfd, const struct iovec __user *, vec,
    ...
    while (iov_iter_count(&iter)) {
        ...
        ret = do_madvise(mm, (unsigned long)iovec.iov_base, iovec.iov_len,
                         behavior);
        if (ret < 0) break;
