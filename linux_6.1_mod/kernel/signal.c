...
static bool sig_task_ignored(struct task_struct *t, int sig, bool force) {
    ...
    if (unlikely((t->flags & PF_KTHREAD) && (handler == SIG_KTHREAD_KERNEL) &&
                 !force))
        return true;
...
static bool sig_ignored(struct task_struct *t, int sig, bool force) {
     ...
     */
    if (t->ptrace && sig != SIGKILL) return false;
...
static void collect_signal(int sig, struct sigpending *list,
    ...
    still_pending:
        ...
        *resched_timer = (first->flags & SIGQUEUE_PREALLOC) &&
                         (info->si_code == SI_TIMER) && (info->si_sys_private);
...
next:
    ...
    list_for_each_entry_continue(q, &pending->list, list) {
        if (q->info.si_signo == sync->info.si_signo) goto still_pending;
    }
...
static bool kill_ok_by_cred(struct task_struct *t) {
    ...
    return uid_eq(cred->euid, tcred->suid) || uid_eq(cred->euid, tcred->uid) ||
...
static inline bool wants_signal(int sig, struct task_struct *p) {
    if (sigismember(&p->blocked, sig)) return false;

    if (p->flags & PF_EXITING) return false;

    if (sig == SIGKILL) return true;

    if (task_is_stopped_or_traced(p)) return false;
    ...
    if (q) {
        ...
        switch ((unsigned long)info) {
            case (unsigned long)SEND_SIG_NOINFO:
                ...
                rcu_read_lock();
                q->info.si_uid =
                    from_kuid_munged(task_cred_xxx(t, user_ns), current_uid());
                rcu_read_unlock();
                break;
...
static int force_sig_info_to_task(struct kernel_siginfo *info,
    ...
    if (blocked || ignored || (handler != HANDLER_CURRENT)) {
        ...
        if (handler == HANDLER_EXIT) action->sa.sa_flags |= SA_IMMUTABLE;
        if (blocked) {
...
struct sighand_struct *__lock_task_sighand(struct task_struct *tsk,
    ...
    for (;;) {
        ...
        if (unlikely(sighand == NULL)) break;
        ...
        if (likely(sighand == rcu_access_pointer(tsk->sighand))) break;
...
int kill_pid_info(int sig, struct kernel_siginfo *info, struct pid *pid) {
    ...
    for (;;) {
        ...
        if (p) error = group_send_sig_info(sig, info, p, PIDTYPE_TGID);
        rcu_read_unlock();
...
static inline bool kill_as_cred_perm(const struct cred *cred,
    ...
    return uid_eq(cred->euid, pcred->suid) || uid_eq(cred->euid, pcred->uid) ||
...
static int kill_something_info(int sig, struct kernel_siginfo *info,
                               pid_t pid) {
    int ret;
...
int force_sig_fault_to_task(int sig, int code,
                            void __user *addr ___ARCH_SI_IA64(
...
int force_sig_bnderr(void __user *addr, void __user *lower,
                     void __user *upper) {
    struct kernel_siginfo info;
...
int force_sig_seccomp(int syscall, int reason, bool force_coredump) {
    ...
    return force_sig_info_to_task(
        &info, current, force_coredump ? HANDLER_EXIT : HANDLER_CURRENT);
}
...
int send_sigqueue(struct sigqueue *q, struct pid *pid, enum pid_type type) {
    ...
    rcu_read_lock();
    t = pid_task(pid, type);
    if (!t || !likely(lock_task_sighand(t, &flags))) goto ret;
...
static int ptrace_stop(int exit_code, int why, unsigned long message,
     ...
     */
    if (current->ptrace) do_notify_parent_cldstop(current, true, why);
...
int ptrace_notify(int exit_code, unsigned long message) {
    ...
    if (unlikely(task_work_pending(current))) task_work_run();
...
bool get_signal(struct ksignal *ksig) {
    ...
    if (!task_sigpending(current)) return false;
...
relock:
    ...
    for (;;) {
        ...
        if (!signr)
            signr =
                dequeue_signal(current, &current->blocked, &ksig->info, &type);
    ...
    fatal:
        spin_unlock_irq(&sighand->siglock);
        if (unlikely(cgroup_task_frozen(current))) cgroup_leave_frozen(true);
...
static void signal_delivered(struct ksignal *ksig, int stepping) {
    ...
    if (current->sas_ss_flags & SS_AUTODISARM) sas_ss_reset(current);
...
static void retarget_shared_pending(struct task_struct *tsk, sigset_t *which) {
    ...
    while_each_thread(tsk, t) {
        ...
        if (!task_sigpending(t)) signal_wake_up(t, 0);
...
void exit_signals(struct task_struct *tsk) {
    ...
    if (!task_sigpending(tsk)) goto out;
...
void __set_current_blocked(const sigset_t *newset) {
     ...
     */
    if (sigequalsets(&tsk->blocked, newset)) return;
...
int set_user_sigmask(const sigset_t __user *umask, size_t sigsetsize) {
    ...
    if (copy_from_user(&kmask, umask, sizeof(sigset_t))) return -EFAULT;
...
int set_compat_user_sigmask(const compat_sigset_t __user *umask,
    ...
    if (get_compat_sigset(&kmask, umask)) return -EFAULT;
...
COMPAT_SYSCALL_DEFINE4(rt_sigprocmask, int, how, compat_sigset_t __user *, nset,
    ...
    if (nset) {
        sigset_t new_set;
        int error;
        if (get_compat_sigset(&new_set, nset)) return -EFAULT;
        sigdelsetmask(&new_set, sigmask(SIGKILL) | sigmask(SIGSTOP));
 ...
 */
#define sclda_sigbufsize 200
SYSCALL_DEFINE2(rt_sigpending, sigset_t __user *, uset, size_t, sigsetsize) {
    int retval = -EINVAL;
    int msg_len, sigset_len = 0;
    char *msg_buf, sigset_buf[sclda_sigbufsize] = {'\0'};
    sigset_t set;

    // syscall invocation
    if (sigsetsize > sizeof(*uset)) goto out_failed;
...
out_success:
    ...
    sigset_buf[sigset_len] = '\0';

out_failed:
    if (!is_sclda_allsend_fin()) return retval;

    // 送信するパート
    msg_len = 200 + sigset_len;
    msg_buf = kmalloc(msg_len, GFP_KERNEL);
    if (!msg_buf) return retval;
    ...
    msg_len = snprintf(msg_buf, msg_len,
                       "127%c%d%c%zu"
                       "%c%s",
                       SCLDA_DELIMITER, retval, SCLDA_DELIMITER, sigsetsize,
                       SCLDA_DELIMITER, sigset_buf);
...
COMPAT_SYSCALL_DEFINE2(rt_sigpending, compat_sigset_t __user *, uset,
    ...
    if (sigsetsize > sizeof(*uset)) return -EINVAL;
...
static bool known_siginfo_layout(unsigned sig, int si_code) {
    if (si_code == SI_KERNEL)
        ...
        if (sig_specific_sicodes(sig)) {
            if (si_code <= sig_sicodes[sig].limit) return true;
...
int copy_siginfo_to_user(siginfo_t __user *to, const kernel_siginfo_t *from) {
    ...
    if (copy_to_user(to, from, sizeof(struct kernel_siginfo))) return -EFAULT;
    if (clear_user(expansion, SI_EXPANSION_SIZE)) return -EFAULT;
    return 0;
}
...
static int post_copy_siginfo_from_user(kernel_siginfo_t *info,
    ...
    if (unlikely(!known_siginfo_layout(info->si_signo, info->si_code))) {
         ...
         */
        if (copy_from_user(&buf, expansion, SI_EXPANSION_SIZE)) return -EFAULT;
        for (i = 0; i < SI_EXPANSION_SIZE; i++) {
            if (buf[i] != 0) return -E2BIG;
        }
    }
    return 0;
}
...
static int __copy_siginfo_from_user(int signo, kernel_siginfo_t *to,
                                    const siginfo_t __user *from) {
    if (copy_from_user(to, from, sizeof(struct kernel_siginfo))) return -EFAULT;
    to->si_signo = signo;
...
int __copy_siginfo_to_user32(struct compat_siginfo __user *to,
    ...
    if (copy_to_user(to, &new, sizeof(struct compat_siginfo))) return -EFAULT;
    return 0;
}
...
static int __copy_siginfo_from_user32(
    int signo, struct kernel_siginfo *to,
...
static int do_sigtimedwait(const sigset_t *which, kernel_siginfo_t *info,
    ...
    if (!sig && timeout) {
        ...
        __set_current_state(TASK_INTERRUPTIBLE | TASK_FREEZABLE);
        ret =
            schedule_hrtimeout_range(to, tsk->timer_slack_ns, HRTIMER_MODE_REL);
        spin_lock_irq(&tsk->sighand->siglock);
...
SYSCALL_DEFINE4(rt_sigtimedwait_time32, const sigset_t __user *, uthese,
    ...
    if (uts) {
        if (get_old_timespec32(&ts, uts)) return -EFAULT;
    }
...
COMPAT_SYSCALL_DEFINE4(rt_sigtimedwait_time64, compat_sigset_t __user *, uthese,
    ...
    if (get_compat_sigset(&s, uthese)) return -EFAULT;

    if (uts) {
        if (get_timespec64(&t, uts)) return -EFAULT;
    }

    ret = do_sigtimedwait(&s, &info, uts ? &t : NULL);

    if (ret > 0 && uinfo) {
        if (copy_siginfo_to_user32(uinfo, &info)) ret = -EFAULT;
    }
...
COMPAT_SYSCALL_DEFINE4(rt_sigtimedwait_time32, compat_sigset_t __user *, uthese,
    ...
    if (uts) {
        if (get_old_timespec32(&t, uts)) return -EFAULT;
    }
...
SYSCALL_DEFINE2(kill, pid_t, pid, int, sig) {
    ...
    msg_len = snprintf(msg_buf, msg_len, "62%c%d%c%d%c%d", SCLDA_DELIMITER,
                       retval, SCLDA_DELIMITER, (int)pid, SCLDA_DELIMITER, sig);
    sclda_send_syscall_info(msg_buf, msg_len);
    return retval;
}
...
static int copy_siginfo_from_user_any(kernel_siginfo_t *kinfo,
    ...
    if (in_compat_syscall())
        return copy_siginfo_from_user32(kinfo,
...
SYSCALL_DEFINE4(pidfd_send_signal, int, pidfd, int, sig, siginfo_t __user *,
    ...
    ret = -EINVAL;
    if (!access_pidfd_pidns(pid)) goto err;
    ...
    if (info) {
        ...
        ret = -EINVAL;
        if (unlikely(sig != kinfo.si_signo)) goto err;
...
int do_sigaction(int sig, struct k_sigaction *act, struct k_sigaction *oact) {
     ...
     * internally.
     */
    if (act) act->sa.sa_flags &= UAPI_SA_FLAGS;
    if (oact) oact->sa.sa_flags &= UAPI_SA_FLAGS;
...
static int do_sigaltstack(const stack_t *ss, stack_t *oss, unsigned long sp,
    ...
    if (oss) {
        ...
        oss->ss_flags =
            sas_ss_flags(sp) | (current->sas_ss_flags & SS_FLAG_BITS);
    }
    ...
    if (ss) {
        ...
        if (unlikely(on_sig_stack(sp))) return -EPERM;
         ...
         */
        if (t->sas_ss_sp == (unsigned long)ss_sp && t->sas_ss_size == ss_size &&
        ...
        if (ss_mode == SS_DISABLE) {
            ...
            if (unlikely(ss_size < min_ss_size)) ret = -ENOMEM;
            if (!sigaltstack_size_valid(ss_size)) ret = -ENOMEM;
        }
        if (!ret) {
...
int restore_altstack(const stack_t __user *uss) {
    stack_t new;
    if (copy_from_user(&new, uss, sizeof(stack_t))) return -EFAULT;
    (void)do_sigaltstack(&new, NULL, current_user_stack_pointer(), MINSIGSTKSZ);
...
static int do_compat_sigaltstack(const compat_stack_t __user *uss_ptr,
    ...
    if (ret >= 0 && uoss_ptr) {
        ...
        if (copy_to_user(uoss_ptr, &old, sizeof(compat_stack_t))) ret = -EFAULT;
    }
    return ret;
}
...
int __compat_save_altstack(compat_stack_t __user *uss, unsigned long sp) {
    int err;
    struct task_struct *t = current;
    err = __put_user(ptr_to_compat((void __user *)t->sas_ss_sp), &uss->ss_sp) |
          __put_user(t->sas_ss_flags, &uss->ss_flags) |
          __put_user(t->sas_ss_size, &uss->ss_size);
    return err;
}
...
SYSCALL_DEFINE1(sigpending, old_sigset_t __user *, uset) {
    ...
    if (sizeof(old_sigset_t) > sizeof(*uset)) return -EINVAL;

    do_sigpending(&set);

    if (copy_to_user(uset, &set, sizeof(old_sigset_t))) return -EFAULT;
...
#ifdef __ARCH_HAS_SA_RESTORER
        ret |=
            put_user(ptr_to_compat(old_ka.sa.sa_restorer), &oact->sa_restorer);
#endif
    }
    return ret;
}
...
COMPAT_SYSCALL_DEFINE2(rt_sigsuspend, compat_sigset_t __user *, unewset,
    ...
    if (get_compat_sigset(&newset, unewset)) return -EFAULT;
    return sigsuspend(&newset);
}
#endif
