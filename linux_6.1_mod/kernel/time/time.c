...
SYSCALL_DEFINE2(gettimeofday, struct __kernel_old_timeval __user *, tv,
    ...
    if (unlikely(tz != NULL))
        if (copy_to_user(tz, &sys_tz, sizeof(sys_tz))) goto sclda;
...
int do_sys_settimeofday64(const struct timespec64 *tv,
    ...
    if (tz) {
        ...
        if (firsttime) {
            firsttime = 0;
            if (!tv) timekeeping_warp_clock();
        }
    }
    if (tv) return do_settimeofday64(tv);
    return 0;
}
...
int get_old_timex32(struct __kernel_timex *txc,
    ...
    if (copy_from_user(&tx32, utp, sizeof(struct old_timex32))) return -EFAULT;
...
int put_old_timex32(struct old_timex32 __user *utp,
    ...
    tx32.tai = txc->tai;
    if (copy_to_user(utp, &tx32, sizeof(struct old_timex32))) return -EFAULT;
    return 0;
}
