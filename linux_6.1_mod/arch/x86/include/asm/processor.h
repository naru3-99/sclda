...
{
	alternative_input(BASE_PREFETCH, "prefetchnta %P1", X86_FEATURE_XMM,
			  "m"(*(const char *)x));
}
...
static __always_inline void prefetchw(const void *x)
{
	alternative_input(BASE_PREFETCH, "prefetchw %P1",
			  X86_FEATURE_3DNOWPREFETCH, "m"(*(const char *)x));
}
...
#define INIT_THREAD                                                   \
	{                                                             \
		.sp0 = TOP_OF_INIT_STACK, .sysenter_cs = __KERNEL_CS, \
	}
...
static inline bool cpu_has_ibpb_brtype_microcode(void)
{
	return false;
}
