...
static bool restore_sigcontext(struct pt_regs *regs,
    ...
    if (copy_from_user(&sc, usc, CONTEXT_COPY_SIZE)) return false;
...
static __always_inline int __unsafe_setup_sigcontext(
    struct sigcontext __user *sc, void __user *fpstate, struct pt_regs *regs,
    unsigned long mask) {
#ifdef CONFIG_X86_32
    unsigned int gs;
    savesegment(gs, gs);
...
#define unsafe_put_sigcontext(sc, fp, regs, set, label)                       \
    do {                                                                      \
        if (__unsafe_setup_sigcontext(sc, fp, regs, set->sig[0])) goto label; \
    } while (0);

#define unsafe_put_sigmask(set, frame, label)                                 \
    unsafe_put_user(*(__u64 *)(set), (__u64 __user *)&(frame)->uc.uc_sigmask, \
                    label)
...
static void __user *get_sigframe(struct k_sigaction *ka, struct pt_regs *regs,
    ...
    /* redzone */
    if (IS_ENABLED(CONFIG_X86_64)) sp -= 128;
    ...
    } else if (IS_ENABLED(CONFIG_X86_32) && !nested_altstack &&
    ...
    sp = fpu__alloc_mathframe(sp, IS_ENABLED(CONFIG_X86_32), &buf_fx,
                              &math_size);
...
static const struct {
    ...
    __NR_sigreturn, 0x80cd, /* int $0x80 */
};
...
__attribute__((packed)) rt_retcode = {0xb8, /* movl $..., %eax */
                                      __NR_rt_sigreturn, 0x80cd, /* int $0x80 */
                                      0};
...
static int __setup_frame(int sig, struct ksignal *ksig, sigset_t *set,
    ...
    if (current->mm->context.vdso)
        restorer =
            current->mm->context.vdso + vdso_image_32.sym___kernel_sigreturn;
    else
        restorer = &frame->retcode;
    if (ksig->ka.sa.sa_flags & SA_RESTORER) restorer = ksig->ka.sa.sa_restorer;
...
static unsigned long frame_uc_flags(struct pt_regs *regs) {
    ...
    if (likely(user_64bit_mode(regs))) flags |= UC_STRICT_RESTORE_SS;
...
static int __setup_rt_frame(int sig, struct ksignal *ksig, sigset_t *set,
    ...
    if (!(ksig->ka.sa.sa_flags & SA_RESTORER)) return -EFAULT;
...
int copy_siginfo_to_user32(struct compat_siginfo __user *to,
                           const struct kernel_siginfo *from) {
    if (in_x32_syscall()) return x32_copy_siginfo_to_user(to, from);
    return __copy_siginfo_to_user32(to, from);
}
#endif /* CONFIG_X86_X32_ABI */

static int x32_setup_rt_frame(struct ksignal *ksig, compat_sigset_t *set,
                              struct pt_regs *regs) {
#ifdef CONFIG_X86_X32_ABI
    ...
    if (ksig->ka.sa.sa_flags & SA_SIGINFO) {
        if (x32_copy_siginfo_to_user(&frame->info, &ksig->info)) return -EFAULT;
    }
...
SYSCALL_DEFINE0(sigreturn) {
     ...
     */
    if (!restore_sigcontext(regs, &frame->sc, 0)) goto badframe;
    return regs->ax;
...
unsigned long sclda_rt_sigreturn(void) {
    struct pt_regs *regs = current_pt_regs();
    struct rt_sigframe __user *frame;
    sigset_t set;
    unsigned long uc_flags;
    ...
    if (__get_user(*(__u64 *)&set, (__u64 __user *)&frame->uc.uc_sigmask))
        goto badframe;
    if (__get_user(uc_flags, &frame->uc.uc_flags)) goto badframe;
...
unsigned long get_sigframe_size(void) { return max_frame_size; }
...
static inline int is_x32_frame(struct ksignal *ksig) {
    return IS_ENABLED(CONFIG_X86_X32_ABI) && ksig->ka.sa.sa_flags & SA_X32_ABI;
}
...
void signal_fault(struct pt_regs *regs, void __user *frame, char *where) {
    ...
    if (show_unhandled_signals && printk_ratelimit()) {
        printk(
            ...
            task_pid_nr(current) > 1 ? KERN_INFO : KERN_EMERG, me->comm,
            me->pid, where, frame, regs->ip, regs->sp, regs->orig_ax);
...
bool sigaltstack_size_valid(size_t ss_size) {
    ...
    if (likely(ss_size > fsize)) return true;

    if (strict_sigaltstack_size) return ss_size > fsize;

    mask = current->group_leader->thread.fpu.perm.__state_perm;
    if (mask & XFEATURE_MASK_USER_DYNAMIC) return ss_size > fsize;
...
COMPAT_SYSCALL_DEFINE0(x32_rt_sigreturn) {
    ...
    if (compat_restore_altstack(&frame->uc.uc_stack)) goto badframe;
