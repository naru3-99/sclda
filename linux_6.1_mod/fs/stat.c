...
int kstat_to_str(struct kstat *stat, char *buffer, size_t max_len) {
    return snprintf(
        ...
        "%llu%c%llu%c%u%c%u",
        stat->result_mask, SCLDA_DELIMITER, (unsigned int)stat->mode,
        SCLDA_DELIMITER, stat->nlink, SCLDA_DELIMITER,
        (unsigned int)stat->blksize, SCLDA_DELIMITER,

        (unsigned long long)stat->attributes, SCLDA_DELIMITER,
        (unsigned long long)stat->attributes_mask, SCLDA_DELIMITER,
        (unsigned long long)stat->ino, SCLDA_DELIMITER,

        (unsigned int)stat->dev, SCLDA_DELIMITER, (unsigned int)stat->rdev,
        SCLDA_DELIMITER, (unsigned int)stat->uid.val, SCLDA_DELIMITER,
        (unsigned int)stat->gid.val, SCLDA_DELIMITER, (long long)stat->size,
        SCLDA_DELIMITER,

        (long long)stat->atime.tv_sec, SCLDA_DELIMITER, stat->atime.tv_nsec,
        SCLDA_DELIMITER, (long long)stat->mtime.tv_sec, SCLDA_DELIMITER,
        stat->mtime.tv_nsec, SCLDA_DELIMITER, (long long)stat->ctime.tv_sec,
        SCLDA_DELIMITER, stat->ctime.tv_nsec, SCLDA_DELIMITER,
        (long long)stat->btime.tv_sec, SCLDA_DELIMITER, stat->btime.tv_nsec,
        SCLDA_DELIMITER,
...
void generic_fill_statx_attr(struct inode *inode, struct kstat *stat) {
    if (inode->i_flags & S_IMMUTABLE) stat->attributes |= STATX_ATTR_IMMUTABLE;
    if (inode->i_flags & S_APPEND) stat->attributes |= STATX_ATTR_APPEND;
...
int vfs_getattr_nosec(const struct path *path, struct kstat *stat,
    ...
    if (inode->i_sb->s_flags & SB_NOATIME) stat->result_mask &= ~STATX_ATIME;
    ...
    if (inode->i_op->getattr)
        return inode->i_op->getattr(mnt_userns, path, stat, request_mask,
                                    query_flags);
...
retry:
    ...
    if (request_mask & STATX_DIOALIGN) {
        ...
        if (S_ISBLK(inode->i_mode)) bdev_statx_dioalign(inode, stat);
    }
...
int vfs_fstatat(int dfd, const char __user *filename, struct kstat *stat,
    ...
    name =
        getname_flags(filename, getname_statx_lookup_flags(statx_flags), NULL);
...
static int cp_old_stat(struct kstat *stat,
    ...
    if (tmp.st_nlink != stat->nlink) return -EOVERFLOW;
    SET_UID(tmp.st_uid, from_kuid_munged(current_user_ns(), stat->uid));
    SET_GID(tmp.st_gid, from_kgid_munged(current_user_ns(), stat->gid));
    tmp.st_rdev = old_encode_dev(stat->rdev);
#if BITS_PER_LONG == 32
    if (stat->size > MAX_NON_LFS) return -EOVERFLOW;
#endif
...
out:
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 ...
                 flag, SCLDA_DELIMITER, struct_buf, SCLDA_DELIMITER, path_buf);
...
SYSCALL_DEFINE4(readlinkat, int, dfd, const char __user *, pathname,
    ...
    retval = do_readlinkat(dfd, pathname, buf, bufsiz);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    // bufを取得する
    buf_len = strnlen_user(buf, PATH_MAX);
    buf_buf = kmalloc(buf_len + 1, GFP_KERNEL);
    if (!buf_buf) goto free_path;
    if (copy_from_user(buf_buf, buf, buf_len)) goto free_buf;
    buf_buf[buf_len] = '\0';

    // 送信するパート
    msg_len = 100 + path_len + buf_len;
    msg_buf = kmalloc(msg_len, GFP_KERNEL);
    if (!msg_buf) goto free_buf;
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 ...
                 "%c%d%c%s%c%s",
                 SCLDA_DELIMITER, retval, SCLDA_DELIMITER, dfd, SCLDA_DELIMITER,
                 bufsiz, SCLDA_DELIMITER, path_buf, SCLDA_DELIMITER, buf_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
SYSCALL_DEFINE3(readlink, const char __user *, path, char __user *, buf, int,
    ...
    path_buf = kmalloc(path_len + 1, GFP_KERNEL);
    if (!path_buf) return retval;
    if (copy_from_user(path_buf, path, path_len)) goto free_path;
    path_buf[path_len] = '\0';
...
SYSCALL_DEFINE2(stat64, const char __user *, filename, struct stat64 __user *,
    ...
    if (!error) error = cp_new_stat64(&stat, statbuf);
 ...
 */
int statx_to_str(const struct statx __user *user_statx, char *buf,
                 int buf_size) {
    struct statx k;
    int len;

    if (copy_from_user(&k, user_statx, sizeof(struct statx))) return -EFAULT;
    ...
    len = snprintf(buf, buf_size,
                   ...
                   "%llu%c%llu%c"
                   "%llu%c%llu%c",
                   k.stx_mask, SCLDA_DELIMITER, k.stx_blksize, SCLDA_DELIMITER,
                   k.stx_attributes, SCLDA_DELIMITER, k.stx_nlink,
                   SCLDA_DELIMITER, k.stx_uid, SCLDA_DELIMITER, k.stx_gid,
                   SCLDA_DELIMITER, k.stx_mode, SCLDA_DELIMITER, k.stx_ino,
                   SCLDA_DELIMITER, k.stx_size, SCLDA_DELIMITER, k.stx_blocks,
                   SCLDA_DELIMITER, k.stx_attributes_mask, SCLDA_DELIMITER);

    // statx_timestampの所(0x80まで)
    len += snprintf(buf + len, buf_size - len, "%lld%c%u%c", k.stx_atime.tv_sec,
                    SCLDA_DELIMITER, k.stx_atime.tv_nsec, SCLDA_DELIMITER);
    len += snprintf(buf + len, buf_size - len, "%lld%c%u%c", k.stx_btime.tv_sec,
                    SCLDA_DELIMITER, k.stx_btime.tv_nsec, SCLDA_DELIMITER);
    len += snprintf(buf + len, buf_size - len, "%lld%c%u%c", k.stx_ctime.tv_sec,
                    SCLDA_DELIMITER, k.stx_ctime.tv_nsec, SCLDA_DELIMITER);
    len += snprintf(buf + len, buf_size - len, "%lld%c%u%c", k.stx_mtime.tv_sec,
                    SCLDA_DELIMITER, k.stx_mtime.tv_nsec, SCLDA_DELIMITER);
...
SYSCALL_DEFINE5(statx, int, dfd, const char __user *, filename, unsigned int,
    ...
    msg_len = snprintf(msg_buf, msg_len,
                       ...
                       SCLDA_DELIMITER, struct_buf, SCLDA_DELIMITER, path_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
static int cp_compat_stat(struct kstat *stat, struct compat_stat __user *ubuf) {
    ...
    SET_GID(tmp.st_gid, from_kgid_munged(current_user_ns(), stat->gid));
    tmp.st_rdev = new_encode_dev(stat->rdev);
    if ((u64)stat->size > MAX_NON_LFS) return -EOVERFLOW;
    tmp.st_size = stat->size;
    tmp.st_atime = stat->atime.tv_sec;
    tmp.st_atime_nsec = stat->atime.tv_nsec;
    tmp.st_mtime = stat->mtime.tv_sec;
...
COMPAT_SYSCALL_DEFINE2(newfstat, unsigned int, fd, struct compat_stat __user *,
    ...
    if (!error) error = cp_compat_stat(&stat, statbuf);
    return error;
}
#endif
