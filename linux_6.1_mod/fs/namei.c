				  ...
				  const struct open_flags *op, bool got_write)
{
	...
	if (open_flag & O_CREAT) {
		if (open_flag & O_EXCL)
			open_flag &= ~O_TRUNC;
		mode = vfs_prepare_mode(mnt_userns, dir->d_inode, mode, mode,
					mode);
		if (likely(got_write))
...
SYSCALL_DEFINE3(mkdirat, int, dfd, const char __user *, pathname, umode_t, mode)
{
	...
	msg_len = snprintf(msg_buf, msg_len, "258%c%d%c%d%c%hu%c%s",
...
SYSCALL_DEFINE1(rmdir, const char __user *, pathname)
{
	...
	msg_len = snprintf(msg_buf, msg_len, "84%c%d%c%s", SCLDA_DELIMITER,
			   retval, SCLDA_DELIMITER, path_buf);
	sclda_send_syscall_info(msg_buf, msg_len);
...
SYSCALL_DEFINE1(unlink, const char __user *, pathname)
{
	...
	msg_len = snprintf(msg_buf, msg_len, "87%c%d%c%s", SCLDA_DELIMITER,
			   retval, SCLDA_DELIMITER, path_buf);
	sclda_send_syscall_info(msg_buf, msg_len);
		...
		const char __user *, newname)
{
	...
	msg_len = snprintf(msg_buf, msg_len, "88%c%d%c%d%c%s%c%s",
			   SCLDA_DELIMITER, retval, SCLDA_DELIMITER, newdfd,
		...
		newname)
{
	...
	msg_len = snprintf(msg_buf, msg_len, "88%c%d%c%s%c%s", SCLDA_DELIMITER,
		...
		newdfd, const char __user *, newname, unsigned int, flags)
{
	...
	msg_len = snprintf(msg_buf, msg_len,
			   ...
			   SCLDA_DELIMITER, retval, SCLDA_DELIMITER, olddfd,
			   SCLDA_DELIMITER, newdfd, SCLDA_DELIMITER, flags,
			   SCLDA_DELIMITER, old_fn_buf, SCLDA_DELIMITER,
			   new_fn_buf);
	sclda_send_syscall_info(msg_buf, msg_len);
		...
		newname)
{
	...
	// syscall invocation
	retval = do_renameat2(AT_FDCWD, getname(oldname), AT_FDCWD,
			      getname(newname), 0);
	if (!is_sclda_allsend_fin())
		return retval;
	...
	msg_len = snprintf(msg_buf, msg_len, "82%c%d%c%s%c%s", SCLDA_DELIMITER,
			   retval, SCLDA_DELIMITER, old_fn_buf, SCLDA_DELIMITER,
			   new_fn_buf);
	sclda_send_syscall_info(msg_buf, msg_len);
