...
struct filename *getname_flags(const char __user *filename, int flags,
    ...
    result = __getname();
    if (unlikely(!result)) return ERR_PTR(-ENOMEM);
...
void putname(struct filename *name) {
    if (IS_ERR(name)) return;

    if (WARN_ON_ONCE(!atomic_read(&name->refcnt))) return;

    if (!atomic_dec_and_test(&name->refcnt)) return;
...
static int check_acl(struct user_namespace *mnt_userns, struct inode *inode,
    ...
    if (mask & MAY_NOT_BLOCK) {
        ...
        if (!acl) return -EAGAIN;
        /* no ->get_acl() calls in RCU mode... */
        if (is_uncached_acl(acl)) return -ECHILD;
...
int inode_permission(struct user_namespace *mnt_userns, struct inode *inode,
    ...
    if (unlikely(mask & MAY_WRITE)) {
         ...
         * to the vfs.
         */
        if (HAS_UNMAPPED_ID(mnt_userns, inode)) return -EACCES;
    }
...
static void restore_nameidata(void) {
    ...
    if (old) old->total_link_count = now->total_link_count;
    if (now->stack != now->internal) kfree(now->stack);
}
...
static bool path_connected(struct vfsmount *mnt, struct dentry *dentry) {
    ...
    if (mnt->mnt_root == sb->s_root) return true;
...
static void terminate_walk(struct nameidata *nd) {
    ...
    if (!(nd->flags & LOOKUP_RCU)) {
        int i;
        path_put(&nd->path);
        for (i = 0; i < nd->depth; i++) path_put(&nd->stack[i].link);
...
static bool legitimize_root(struct nameidata *nd) {
    ...
    if (!nd->root.mnt || (nd->state & ND_ROOT_PRESET)) return true;
    nd->state |= ND_ROOT_GRABBED;
...
static bool try_to_unlazy(struct nameidata *nd) {
    ...
    if (unlikely(!legitimize_links(nd))) goto out1;
    if (unlikely(!legitimize_path(nd, &nd->path, nd->seq))) goto out;
    if (unlikely(!legitimize_root(nd))) goto out;
    leave_rcu(nd);
 ...
 * context. Nothing should touch nameidata between try_to_unlazy_next() failure
 * and terminate_walk().
 */
static bool try_to_unlazy_next(struct nameidata *nd, struct dentry *dentry) {
     ...
     */
    if (unlikely(!lockref_get_not_dead(&dentry->d_lockref))) goto out;
    if (read_seqcount_retry(&dentry->d_seq, nd->next_seq)) goto out_dput;
    /*
...
static int complete_walk(struct nameidata *nd) {
    ...
    if (nd->flags & LOOKUP_RCU) {
        ...
        if (!(nd->state & ND_ROOT_PRESET))
            if (!(nd->flags & LOOKUP_IS_SCOPED)) nd->root.mnt = NULL;
...
static int nd_jump_root(struct nameidata *nd) {
    if (unlikely(nd->flags & LOOKUP_BENEATH)) return -EXDEV;
    if (unlikely(nd->flags & LOOKUP_NO_XDEV)) {
        /* Absolute path arguments to path_init() are allowed. */
        if (nd->path.mnt != NULL && nd->path.mnt != nd->root.mnt) return -EXDEV;
    }
    if (!nd->root.mnt) {
    ...
    if (nd->flags & LOOKUP_RCU) {
        ...
        nd->inode = d->d_inode;
        nd->seq = nd->root_seq;
        if (read_seqcount_retry(&d->d_seq, nd->seq)) return -ECHILD;
    } else {
        path_put(&nd->path);
        nd->path = nd->root;
        path_get(&nd->path);
...
int nd_jump_link(const struct path *path) {
    ...
    error = -EXDEV;
    if (unlikely(nd->flags & LOOKUP_NO_XDEV)) {
        if (nd->path.mnt != path->mnt) goto err;
    }
    /* Not currently safe for scoped-lookups. */
    if (unlikely(nd->flags & LOOKUP_IS_SCOPED)) goto err;
...
static inline void put_link(struct nameidata *nd) {
    ...
    do_delayed_call(&last->done);
    if (!(nd->flags & LOOKUP_RCU)) path_put(&last->link);
}
...
static inline int may_follow_link(struct nameidata *nd,
    ...
    if (uid_eq(current_cred()->fsuid, i_uid)) return 0;

    /* Allowed if parent directory not sticky and world-writable. */
    if ((nd->dir_mode & (S_ISVTX | S_IWOTH)) != (S_ISVTX | S_IWOTH)) return 0;

    /* Allowed if parent directory and link owner match. */
    if (uid_valid(nd->dir_uid) && uid_eq(nd->dir_uid, i_uid)) return 0;
...
static bool safe_hardlink_source(struct user_namespace *mnt_userns,
    ...
    if (mode & S_ISUID) return false;

    /* Executable setgid files should not get pinned to the filesystem. */
    if ((mode & (S_ISGID | S_IXGRP)) == (S_ISGID | S_IXGRP)) return false;

    /* Hardlinking to unreadable or unwritable sources is dangerous. */
    if (inode_permission(mnt_userns, inode, MAY_READ | MAY_WRITE)) return false;
...
static bool choose_mountpoint_rcu(struct mount *m, const struct path *root,
    ...
    while (mnt_has_parent(m)) {
        ...
        m = m->mnt_parent;
        if (unlikely(root->dentry == mountpoint && root->mnt == &m->mnt)) break;
...
static bool choose_mountpoint(struct mount *m, const struct path *root,
    ...
    while (1) {
        ...
        if (unlikely(!found)) {
            if (!read_seqretry(&mount_lock, mseq)) break;
        } else {
            if (likely(__legitimize_path(path, seq, mseq))) break;
            rcu_read_unlock();
            path_put(path);
            rcu_read_lock();
        }
...
static int __traverse_mounts(struct path *path, unsigned flags, bool *jumped,
    ...
    if (need_mntput && path->mnt == mnt) mntput(path->mnt);
    if (!ret && unlikely(d_flags_negative(flags))) ret = -ENOENT;
    *jumped = need_mntput;
    return ret;
}
...
static inline int traverse_mounts(struct path *path, bool *jumped, int *count,
    ...
    if (likely(!(flags & DCACHE_MANAGED_DENTRY))) {
        *jumped = false;
        if (unlikely(d_flags_negative(flags))) return -ENOENT;
        return 0;
    }
...
static bool __follow_mount_rcu(struct nameidata *nd, struct path *path) {
    ...
    if (likely(!(flags & DCACHE_MANAGED_DENTRY))) return true;

    if (unlikely(nd->flags & LOOKUP_NO_XDEV)) return false;
    ...
    for (;;) {
        ...
        if (flags & DCACHE_MOUNTED) {
            ...
            if (mounted) {
                ...
                // this state.
                if (read_seqretry(&mount_lock, nd->m_seq)) return false;
                continue;
            }
            if (read_seqretry(&mount_lock, nd->m_seq)) return false;
        }
...
static inline int handle_mounts(struct nameidata *nd, struct dentry *dentry,
    ...
    if (nd->flags & LOOKUP_RCU) {
        ...
        if (likely(__follow_mount_rcu(nd, path))) return 0;
        // *path and nd->next_seq might've been clobbered
        path->mnt = nd->path.mnt;
        path->dentry = dentry;
        nd->next_seq = seq;
        if (!try_to_unlazy_next(nd, dentry)) return -ECHILD;
    }
    ...
    if (unlikely(ret)) {
        dput(path->dentry);
        if (path->mnt != nd->path.mnt) mntput(path->mnt);
    }
    return ret;
}
...
static struct dentry *__lookup_hash(const struct qstr *name,
    ...
    if (unlikely(IS_DEADDIR(dir))) return ERR_PTR(-ENOENT);

    dentry = d_alloc(base, name);
    if (unlikely(!dentry)) return ERR_PTR(-ENOMEM);
...
static struct dentry *lookup_fast(struct nameidata *nd) {
    ...
    if (nd->flags & LOOKUP_RCU) {
        ...
        if (likely(status > 0)) return dentry;
        if (!try_to_unlazy_next(nd, dentry)) return ERR_PTR(-ECHILD);
        if (status == -ECHILD) /* we'd been told to redo it in non-rcu mode */
            status = d_revalidate(dentry, nd->flags);
    } else {
        dentry = __d_lookup(parent, &nd->last);
        if (unlikely(!dentry)) return NULL;
        status = d_revalidate(dentry, nd->flags);
    }
...
static struct dentry *__lookup_slow(const struct qstr *name, struct dentry *dir,
    ...
    if (unlikely(IS_DEADDIR(inode))) return ERR_PTR(-ENOENT);
again:
...
static inline int may_lookup(struct user_namespace *mnt_userns,
    ...
    if (nd->flags & LOOKUP_RCU) {
        int err =
            inode_permission(mnt_userns, nd->inode, MAY_EXEC | MAY_NOT_BLOCK);
        if (err != -ECHILD || !try_to_unlazy(nd)) return err;
    }
    return inode_permission(mnt_userns, nd->inode, MAY_EXEC);
}

static int reserve_stack(struct nameidata *nd, struct path *link) {
    if (unlikely(nd->total_link_count++ >= MAXSYMLINKS)) return -ELOOP;

    if (likely(nd->depth != EMBEDDED_LEVELS)) return 0;
...
static const char *pick_link(struct nameidata *nd, struct path *link,
    ...
    error =
        security_inode_follow_link(link->dentry, inode, nd->flags & LOOKUP_RCU);
    if (unlikely(error)) return ERR_PTR(error);
    ...
    if (likely(!d_is_symlink(path.dentry)) ||
        ...
        if (nd->flags & LOOKUP_RCU) {
            if (read_seqcount_retry(&path.dentry->d_seq, nd->next_seq))
                return ERR_PTR(-ECHILD);
            if (unlikely(!inode)) return ERR_PTR(-ENOENT);
        } else {
            dput(nd->path.dentry);
            if (nd->path.mnt != path.mnt) mntput(nd->path.mnt);
        }
        nd->path = path;
        nd->inode = inode;
        nd->seq = nd->next_seq;
...
static struct dentry *follow_dotdot_rcu(struct nameidata *nd) {
    ...
    if (path_equal(&nd->path, &nd->root)) goto in_root;
    if (unlikely(nd->path.dentry == nd->path.mnt->mnt_root)) {
        struct path path;
        unsigned seq;
        if (!choose_mountpoint_rcu(real_mount(nd->path.mnt), &nd->root, &path,
                                   &seq))
            goto in_root;
        if (unlikely(nd->flags & LOOKUP_NO_XDEV)) return ERR_PTR(-ECHILD);
        nd->path = path;
        nd->inode = path.dentry->d_inode;
        nd->seq = seq;
        // makes sure that non-RCU pathwalk could reach this state
        if (read_seqretry(&mount_lock, nd->m_seq)) return ERR_PTR(-ECHILD);
    ...
    // makes sure that non-RCU pathwalk could reach this state
    if (read_seqcount_retry(&old->d_seq, nd->seq)) return ERR_PTR(-ECHILD);
...
static const char *walk_component(struct nameidata *nd, int flags) {
    ...
    if (unlikely(nd->last_type != LAST_NORM)) {
        if (!(flags & WALK_MORE) && nd->depth) put_link(nd);
...
unsigned int full_name_hash(const void *salt, const char *name,
    ...
    for (;;) {
        ...
        if (len < sizeof(unsigned long)) break;
        HASH_MIX(x, y, a);
...
static inline u64 hash_name(const void *salt, const char *name) {
    ...
    inside:
        ...
        !(has_zero(a, &adata, &constants) | has_zero(b, &bdata, &constants)));
...
static const char *path_init(struct nameidata *nd, unsigned flags) {
    ...
    if (nd->state & ND_ROOT_PRESET) {
        ...
        if (*s && unlikely(!d_can_lookup(root))) return ERR_PTR(-ENOTDIR);
        nd->path = nd->root;
        nd->inode = inode;
        if (flags & LOOKUP_RCU) {
...
static int handle_lookup_down(struct nameidata *nd) {
    if (!(nd->flags & LOOKUP_RCU)) dget(nd->path.dentry);
    nd->next_seq = nd->seq;
...
static int path_lookupat(struct nameidata *nd, unsigned flags,
    ...
    if (unlikely(flags & LOOKUP_DOWN) && !IS_ERR(s)) {
        ...
        if (unlikely(err < 0)) s = ERR_PTR(err);
    }
    ...
    if (!err && nd->flags & LOOKUP_DIRECTORY)
        if (!d_can_lookup(nd->path.dentry)) err = -ENOTDIR;
    if (!err) {
        *path = nd->path;
        nd->path.mnt = NULL;
        nd->path.dentry = NULL;
...
int filename_lookup(int dfd, struct filename *name, unsigned flags,
    ...
    if (unlikely(retval == -ECHILD)) retval = path_lookupat(&nd, flags, path);
...
static int filename_parentat(int dfd, struct filename *name, unsigned int flags,
    ...
    if (unlikely(retval == -ECHILD)) retval = path_parentat(&nd, flags, parent);
    if (unlikely(retval == -ESTALE))
...
static int lookup_one_common(struct user_namespace *mnt_userns,
    ...
    if (!len) return -EACCES;

    if (unlikely(name[0] == '.')) {
        if (len < 2 || (len == 2 && name[1] == '.')) return -EACCES;
    }

    while (len--) {
        unsigned int c = *(const unsigned char *)name++;
        if (c == '/' || c == '\0') return -EACCES;
    }
    /*
...
struct dentry *lookup_one_unlocked(struct user_namespace *mnt_userns,
    ...
    if (!ret) ret = lookup_slow(&this, base, 0);
    return ret;
}
...
int path_pts(struct path *path) {
    ...
    if (IS_ERR_OR_NULL(child)) return -ENOENT;
...
int __check_sticky(struct user_namespace *mnt_userns, struct inode *dir,
    ...
    if (uid_eq(i_uid_into_mnt(mnt_userns, inode), fsuid)) return 0;
...
static int may_delete(struct user_namespace *mnt_userns, struct inode *dir,
    ...
    if (d_is_negative(victim)) return -ENOENT;
    BUG_ON(!inode);
    ...
    if (check_sticky(mnt_userns, dir, inode) || IS_APPEND(inode) ||
        ...
        return -EPERM;
    if (isdir) {
        if (!d_is_dir(victim)) return -ENOTDIR;
        if (IS_ROOT(victim)) return -EBUSY;
    } else if (d_is_dir(victim))
        return -EISDIR;
    if (IS_DEADDIR(dir)) return -ENOENT;
    if (victim->d_flags & DCACHE_NFSFS_RENAMED) return -EBUSY;
    return 0;
}
...
static inline int may_create(struct user_namespace *mnt_userns,
    ...
    if (IS_DEADDIR(dir)) return -ENOENT;
    if (!fsuidgid_has_mapping(dir->i_sb, mnt_userns)) return -EOVERFLOW;
...
struct dentry *lock_rename(struct dentry *p1, struct dentry *p2) {
    ...
    lock_two_inodes(p1->d_inode, p2->d_inode, I_MUTEX_PARENT, I_MUTEX_PARENT2);
    return NULL;
}
...
static inline umode_t mode_strip_umask(const struct inode *dir, umode_t mode) {
    if (!IS_POSIXACL(dir)) mode &= ~current_umask();
    return mode;
}
...
int vfs_create(struct user_namespace *mnt_userns, struct inode *dir,
    ...
    if (!error) fsnotify_create(dir, dentry);
    return error;
}
...
static int may_open(struct user_namespace *mnt_userns, const struct path *path,
    ...
    switch (inode->i_mode & S_IFMT) {
        ...
        case S_IFDIR:
            ...
            if (acc_mode & MAY_EXEC) return -EACCES;
            break;
        case S_IFBLK:
        case S_IFCHR:
            if (!may_open_dev(path)) return -EACCES;
            fallthrough;
        case S_IFIFO:
        case S_IFSOCK:
            if (acc_mode & MAY_EXEC) return -EACCES;
            ...
            break;
        case S_IFREG:
            if ((acc_mode & MAY_EXEC) && path_noexec(path)) return -EACCES;
            break;
    }
     ...
     */
    if (IS_APPEND(inode)) {
        if ((flag & O_ACCMODE) != O_RDONLY && !(flag & O_APPEND)) return -EPERM;
        if (flag & O_TRUNC) return -EPERM;
    }
...
static int handle_truncate(struct user_namespace *mnt_userns,
                           struct file *filp) {
...
static int may_o_create(struct user_namespace *mnt_userns,
    ...
    if (!fsuidgid_has_mapping(dir->dentry->d_sb, mnt_userns)) return -EOVERFLOW;
...
static struct dentry *atomic_open(struct nameidata *nd, struct dentry *dentry,
    ...
    if (nd->flags & LOOKUP_DIRECTORY) open_flag |= O_DIRECTORY;
    ...
    if (!error) {
        if (file->f_mode & FMODE_OPENED) {
            ...
            if (unlikely(d_is_negative(dentry))) error = -ENOENT;
        }
    }
    if (error) {
        dput(dentry);
...
static struct dentry *lookup_open(struct nameidata *nd, struct file *file,
    ...
    if (unlikely(IS_DEADDIR(dir_inode))) return ERR_PTR(-ENOENT);
    ...
    for (;;) {
        ...
        if (d_in_lookup(dentry)) break;

        error = d_revalidate(dentry, nd->flags);
        if (likely(error > 0)) break;
     ...
     */
    if (unlikely(!got_write)) open_flag &= ~O_TRUNC;
    mnt_userns = mnt_user_ns(nd->path.mnt);
    ...
    if (create_error) open_flag &= ~O_CREAT;
    ...
    if (!dentry->d_inode && (open_flag & O_CREAT)) {
        ...
        error = dir_inode->i_op->create(mnt_userns, dir_inode, dentry, mode,
                                        open_flag & O_EXCL);
        if (error) goto out_dput;
    }
...
static const char *open_last_lookups(struct nameidata *nd, struct file *file,
    ...
    if (got_write) mnt_drop_write(nd->path.mnt);
...
static int do_open(struct nameidata *nd, struct file *file,
    ...
    if (open_flag & O_CREAT) {
        if ((open_flag & O_EXCL) && !(file->f_mode & FMODE_CREATED))
            return -EEXIST;
        if (d_is_dir(nd->path.dentry)) return -EISDIR;
    ...
    if (!error && do_truncate) error = handle_truncate(mnt_userns, file);
    if (unlikely(error > 0)) {
        WARN_ON(1);
        error = -EINVAL;
    }
...
static int vfs_tmpfile(struct user_namespace *mnt_userns,
                       const struct path *parentpath, struct file *file,
                       umode_t mode) {
    struct dentry *child;
    ...
    error = inode_permission(mnt_userns, dir, MAY_WRITE | MAY_EXEC);
    if (error) return error;
    if (!dir->i_op->tmpfile) return -EOPNOTSUPP;
...
static struct file *path_openat(struct nameidata *nd,
    ...
    if (likely(!error)) {
        if (likely(file->f_mode & FMODE_OPENED)) return file;
        WARN_ON(1);
        error = -EINVAL;
    }
    fput(file);
...
struct file *do_filp_open(int dfd, struct filename *pathname,
    ...
    if (unlikely(filp == ERR_PTR(-ECHILD))) filp = path_openat(&nd, op, flags);
...
struct file *do_file_open_root(const struct path *root, const char *name,
    ...
    filename = getname_kernel(name);
    if (IS_ERR(filename)) return ERR_CAST(filename);

    set_nameidata(&nd, -1, filename, root);
    file = path_openat(&nd, op, flags | LOOKUP_RCU);
    if (unlikely(file == ERR_PTR(-ECHILD))) file = path_openat(&nd, op, flags);
...
static struct dentry *filename_create(int dfd, struct filename *name,
     ...
     * (foo/., foo/.., /////)
     */
    if (unlikely(type != LAST_NORM)) goto out;
     ...
     */
    if (last.name[last.len] && !want_dir) create_flags = 0;
    inode_lock_nested(path->dentry->d_inode, I_MUTEX_PARENT);
    dentry = __lookup_hash(&last, path->dentry, reval_flag | create_flags);
    if (IS_ERR(dentry)) goto unlock;

    error = -EEXIST;
    if (d_is_positive(dentry)) goto fail;
...
unlock:
    inode_unlock(path->dentry->d_inode);
    if (!err2) mnt_drop_write(path->mnt);
out:
    path_put(path);
    return dentry;
}
...
int vfs_mknod(struct user_namespace *mnt_userns, struct inode *dir,
    ...
    if (!dir->i_op->mknod) return -EPERM;
...
retry:
    ...
    if (IS_ERR(dentry)) goto out1;

    error = security_path_mknod(
        &path, dentry, mode_strip_umask(path.dentry->d_inode, mode), dev);
    if (error) goto out2;
    ...
    switch (mode & S_IFMT) {
        case 0:
        case S_IFREG:
            error = vfs_create(mnt_userns, path.dentry->d_inode, dentry, mode,
                               true);
            if (!error) ima_post_path_mknod(mnt_userns, dentry);
            break;
        case S_IFCHR:
        case S_IFBLK:
            error = vfs_mknod(mnt_userns, path.dentry->d_inode, dentry, mode,
                              new_decode_dev(dev));
            break;
        case S_IFIFO:
        case S_IFSOCK:
            error =
                vfs_mknod(mnt_userns, path.dentry->d_inode, dentry, mode, 0);
            break;
    }
out2:
...
SYSCALL_DEFINE4(mknodat, int, dfd, const char __user *, filename, umode_t, mode,
    ...
    retval = do_mknodat(dfd, getname(filename), mode, dev);
    ...
    msg_len = snprintf(msg_buf, msg_len,
                       ...
                       SCLDA_DELIMITER, (unsigned short)mode, SCLDA_DELIMITER,
                       dev, SCLDA_DELIMITER, path_buf);
...
SYSCALL_DEFINE3(mknod, const char __user *, filename, umode_t, mode, unsigned,
    ...
    retval = do_mknodat(AT_FDCWD, getname(filename), mode, dev);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 "133%c%d%c%u"
                 "%c%u%c%s",
                 SCLDA_DELIMITER, retval, SCLDA_DELIMITER, (unsigned short)mode,
                 SCLDA_DELIMITER, dev, SCLDA_DELIMITER, path_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
retry:
    dentry = filename_create(dfd, name, &path, lookup_flags);
    error = PTR_ERR(dentry);
    if (IS_ERR(dentry)) goto out_putname;
...
SYSCALL_DEFINE3(mkdirat, int, dfd, const char __user *, pathname, umode_t,
    ...
    retval = do_mkdirat(dfd, getname(pathname), mode);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    path_buf = kmalloc(path_len + 1, GFP_KERNEL);
    if (!path_buf) return retval;
    if (copy_from_user(path_buf, pathname, path_len)) goto free_path;
    path_buf[path_len] = '\0';
    ...
    msg_len =
        snprintf(msg_buf, msg_len, "258%c%d%c%d%c%hu%c%s", SCLDA_DELIMITER,
                 retval, SCLDA_DELIMITER, dfd, SCLDA_DELIMITER,
...
int vfs_rmdir(struct user_namespace *mnt_userns, struct inode *dir,
    ...
    if (!dir->i_op->rmdir) return -EPERM;
...
out:
    ...
    dput(dentry);
    if (!error) d_delete_notify(dir, dentry);
    return error;
}
...
SYSCALL_DEFINE1(rmdir, const char __user *, pathname) {
    ...
    retval = do_rmdir(AT_FDCWD, getname(pathname));
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len = snprintf(msg_buf, msg_len, "84%c%d%c%s", SCLDA_DELIMITER, retval,
                       SCLDA_DELIMITER, path_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
retry_deleg:
    ...
    if (!IS_ERR(dentry)) {
        ...
        if (d_is_negative(dentry)) goto slashes;
        ihold(inode);
...
SYSCALL_DEFINE1(unlink, const char __user *, pathname) {
    ...
    retval = do_unlinkat(AT_FDCWD, getname(pathname));
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len = snprintf(msg_buf, msg_len, "87%c%d%c%s", SCLDA_DELIMITER, retval,
                       SCLDA_DELIMITER, path_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
int vfs_symlink(struct user_namespace *mnt_userns, struct inode *dir,
    ...
    if (!dir->i_op->symlink) return -EPERM;
...
retry:
    ...
    error = PTR_ERR(dentry);
    if (IS_ERR(dentry)) goto out_putnames;
    ...
    if (!error) {
        ...
        mnt_userns = mnt_user_ns(path.mnt);
        error =
            vfs_symlink(mnt_userns, path.dentry->d_inode, dentry, from->name);
    }
    done_path_create(&path, dentry);
    if (retry_estale(error, lookup_flags)) {
        lookup_flags |= LOOKUP_REVAL;
...
SYSCALL_DEFINE3(symlinkat, const char __user *, oldname, int, newdfd,
    ...
    retval = do_symlinkat(getname(oldname), newdfd, getname(newname));
    if (!is_sclda_allsend_fin()) return retval;
...
SYSCALL_DEFINE2(symlink, const char __user *, oldname, const char __user *,
    ...
    retval = do_symlinkat(getname(oldname), AT_FDCWD, getname(newname));
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len =
        snprintf(msg_buf, msg_len, "88%c%d%c%s%c%s", SCLDA_DELIMITER, retval,
...
int vfs_link(struct dentry *old_dentry, struct user_namespace *mnt_userns,
     ...
     * the vfs.
     */
    if (HAS_UNMAPPED_ID(mnt_userns, inode)) return -EPERM;
    if (!dir->i_op->link) return -EPERM;
    ...
    inode_unlock(inode);
    if (!error) fsnotify_link(dir, inode, new_dentry);
    return error;
}
...
retry:
    ...
    new_dentry = filename_create(newdfd, new, &new_path, (how & LOOKUP_REVAL));
    error = PTR_ERR(new_dentry);
    if (IS_ERR(new_dentry)) goto out_putpath;

    error = -EXDEV;
    if (old_path.mnt != new_path.mnt) goto out_dput;
    mnt_userns = mnt_user_ns(new_path.mnt);
    error = may_linkat(mnt_userns, &old_path);
    if (unlikely(error)) goto out_dput;
...
SYSCALL_DEFINE5(linkat, int, olddfd, const char __user *, oldname, int, newdfd,
    ...
    retval = do_linkat(olddfd, getname_uflags(oldname, flags), newdfd,
    ...
    msg_len = snprintf(msg_buf, msg_len,
                       ...
                       SCLDA_DELIMITER, newdfd, SCLDA_DELIMITER, flags,
                       SCLDA_DELIMITER, old_buf, SCLDA_DELIMITER, new_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
int vfs_rename(struct renamedata *rd) {
    ...
    if (!target) {
        ...
        if (!(flags & RENAME_EXCHANGE))
            ...
            error =
                may_delete(rd->new_mnt_userns, new_dir, new_dentry, new_is_dir);
    }
    if (error) return error;
    ...
    error = -EPERM;
    if (IS_SWAPFILE(source) || (target && IS_SWAPFILE(target))) goto out;
    ...
    if (max_links && new_dir != old_dir) {
        error = -EMLINK;
        if (is_dir && !new_is_dir && new_dir->i_nlink >= max_links) goto out;
...
out:
    inode_unlock(source);
    if (target) inode_unlock(target);
    dput(new_dentry);
    if (!error) {
        fsnotify_move(old_dir, new_dir, &old_name.name, is_dir,
                      !(flags & RENAME_EXCHANGE) ? target : NULL, old_dentry);
        if (flags & RENAME_EXCHANGE) {
            fsnotify_move(new_dir, old_dir, &old_dentry->d_name, new_is_dir,
                          NULL, new_dentry);
        }
    }
...
retry:
    error = filename_parentat(olddfd, from, lookup_flags, &old_path, &old_last,
                              &old_type);
    if (error) goto put_names;
    ...
    error = -EXDEV;
    if (old_path.mnt != new_path.mnt) goto exit2;

    error = -EBUSY;
    if (old_type != LAST_NORM) goto exit2;

    if (flags & RENAME_NOREPLACE) error = -EEXIST;
    if (new_type != LAST_NORM) goto exit2;
...
retry_deleg:
    ...
    if (IS_ERR(old_dentry)) goto exit3;
    /* source must exist */
    error = -ENOENT;
    if (d_is_negative(old_dentry)) goto exit4;
    new_dentry =
        __lookup_hash(&new_last, new_path.dentry, lookup_flags | target_flags);
    error = PTR_ERR(new_dentry);
    if (IS_ERR(new_dentry)) goto exit4;
    error = -EEXIST;
    if ((flags & RENAME_NOREPLACE) && d_is_positive(new_dentry)) goto exit5;
    if (flags & RENAME_EXCHANGE) {
        error = -ENOENT;
        if (d_is_negative(new_dentry)) goto exit5;

        if (!d_is_dir(new_dentry)) {
            error = -ENOTDIR;
            if (new_last.name[new_last.len]) goto exit5;
        }
    }
    /* unless the source is a directory trailing slashes give -ENOTDIR */
    if (!d_is_dir(old_dentry)) {
        error = -ENOTDIR;
        if (old_last.name[old_last.len]) goto exit5;
        if (!(flags & RENAME_EXCHANGE) && new_last.name[new_last.len])
            goto exit5;
    }
    /* source should not be ancestor of target */
    error = -EINVAL;
    if (old_dentry == trap) goto exit5;
    /* target should not be an ancestor of source */
    if (!(flags & RENAME_EXCHANGE)) error = -ENOTEMPTY;
    if (new_dentry == trap) goto exit5;

    error = security_path_rename(&old_path, old_dentry, &new_path, new_dentry,
                                 flags);
    if (error) goto exit5;
...
SYSCALL_DEFINE5(renameat2, int, olddfd, const char __user *, oldname, int,
    ...
    // syscall invocation
    retval =
        do_renameat2(olddfd, getname(oldname), newdfd, getname(newname), flags);
    if (!is_sclda_allsend_fin()) return retval;

    // oldnameを取得
    old_fn_len = strnlen_user(oldname, PATH_MAX);
    old_fn_buf = kmalloc(old_fn_len + 1, GFP_KERNEL);
    if (!old_fn_buf) return retval;
    if (copy_from_user(old_fn_buf, oldname, old_fn_len)) goto free_old;
    old_fn_buf[old_fn_len] = '\0';

    // newnameを取得
    new_fn_len = strnlen_user(newname, PATH_MAX);
    new_fn_buf = kmalloc(new_fn_len + 1, GFP_KERNEL);
    if (!new_fn_buf) goto free_old;
    if (copy_from_user(new_fn_buf, newname, new_fn_len)) goto free_new;
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 ...
                 SCLDA_DELIMITER, retval, SCLDA_DELIMITER, olddfd,
                 SCLDA_DELIMITER, newdfd, SCLDA_DELIMITER, flags,
                 SCLDA_DELIMITER, old_fn_buf, SCLDA_DELIMITER, new_fn_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
SYSCALL_DEFINE4(renameat, int, olddfd, const char __user *, oldname, int,
    ...
    // syscall invocation
    retval =
        do_renameat2(olddfd, getname(oldname), newdfd, getname(newname), 0);
    if (!is_sclda_allsend_fin()) return retval;
...
SYSCALL_DEFINE2(rename, const char __user *, oldname, const char __user *,
    ...
    // syscall invocation
    retval =
        do_renameat2(AT_FDCWD, getname(oldname), AT_FDCWD, getname(newname), 0);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len =
        snprintf(msg_buf, msg_len, "82%c%d%c%s%c%s", SCLDA_DELIMITER, retval,
                 SCLDA_DELIMITER, old_fn_buf, SCLDA_DELIMITER, new_fn_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
int readlink_copy(char __user *buffer, int buflen, const char *link) {
    ...
    if (IS_ERR(link)) goto out;

    len = strlen(link);
    if (len > (unsigned)buflen) len = buflen;
    if (copy_to_user(buffer, link, len)) len = -EFAULT;
out:
    return len;
}
...
int vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen) {
    ...
    if (unlikely(!(inode->i_opflags & IOP_DEFAULT_READLINK))) {
        ...
        if (!d_is_symlink(dentry)) return -EINVAL;
...
const char *vfs_get_link(struct dentry *dentry, struct delayed_call *done) {
    ...
    if (d_is_symlink(dentry)) {
        ...
        if (!res) res = inode->i_op->get_link(dentry, inode, done);
    }
    return res;
}
...
const char *page_get_link(struct dentry *dentry, struct inode *inode,
    ...
    if (!dentry) {
        ...
        if (IS_ERR(page)) return (char *)page;
    }
...
retry:
    if (nofs) flags = memalloc_nofs_save();
    err = aops->write_begin(NULL, mapping, 0, len - 1, &page, &fsdata);
    if (nofs) memalloc_nofs_restore(flags);
    if (err) goto fail;
