...
long vfs_truncate(const struct path *path, loff_t length) {
    ...
    error = -EPERM;
    if (IS_APPEND(inode)) goto mnt_drop_write_and_out;
    ...
    if (error) goto put_write_and_out;

    error = security_path_truncate(path);
    if (!error) error = do_truncate(mnt_userns, path->dentry, length, 0, NULL);
...
SYSCALL_DEFINE2(truncate, const char __user *, path, long, length) {
    ...
    if (copy_from_user(filename_buf, path, filename_len)) filename_len = 0;
    ...
    msg_len =
        snprintf(msg_buf, msg_len, "76%c%ld%c%ld%c%s", SCLDA_DELIMITER, retval,
                 SCLDA_DELIMITER, length, SCLDA_DELIMITER, filename_buf);
...
long do_sys_ftruncate(unsigned int fd, loff_t length, int small) {
    ...
    if (f.file->f_flags & O_LARGEFILE) small = 0;
    ...
    if (small && length > MAX_NON_LFS) goto out_putf;

    error = -EPERM;
    /* Check IS_APPEND on real upper inode */
    if (IS_APPEND(file_inode(f.file))) goto out_putf;
...
SYSCALL_DEFINE2(ftruncate, unsigned int, fd, unsigned long, length) {
    ...
    retval = do_sys_ftruncate(fd, length, 1);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len = snprintf(msg_buf, msg_len, "77%c%ld%c%u%c%lu", SCLDA_DELIMITER,
                       retval, SCLDA_DELIMITER, fd, SCLDA_DELIMITER, length);
    sclda_send_syscall_info(msg_buf, msg_len);
    return retval;
}
...
int vfs_fallocate(struct file *file, int mode, loff_t offset, loff_t len) {
    ...
    if (mode & ~FALLOC_FL_SUPPORTED_MASK) return -EOPNOTSUPP;
...
retry:
    ...
    if ((mode & MAY_EXEC) && S_ISREG(inode->i_mode)) {
         ...
         */
        res = -EACCES;
        if (path_noexec(&path)) goto out_path_release;
    }
     ...
     */
    if (__mnt_is_readonly(path.mnt)) res = -EROFS;
...
SYSCALL_DEFINE3(faccessat, int, dfd, const char __user *, filename, int, mode) {
    ...
    if (!file_buf) return retval;
    if (copy_from_user(file_buf, filename, file_len)) goto free_file;
    file_buf[file_len] = '\0';

    // 送信するパート
    msg_len = 150 + file_len;
    msg_buf = kmalloc(msg_len, GFP_KERNEL);
    if (!msg_buf) goto free_file;
...
SYSCALL_DEFINE4(faccessat2, int, dfd, const char __user *, filename, int, mode,
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 ...
                 mode, SCLDA_DELIMITER, flags, SCLDA_DELIMITER, file_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
SYSCALL_DEFINE1(fchdir, unsigned int, fd) {
    ...
    error = -ENOTDIR;
    if (!d_can_lookup(f.file->f_path.dentry)) goto out_putf;

    error = file_permission(f.file, MAY_EXEC | MAY_CHDIR);
    if (!error) set_fs_pwd(current->fs, &f.file->f_path);
out_putf:
    fdput(f);
out:
    int msg_len;
    ...
    msg_len = 100;
    msg_buf = kmalloc(msg_len, GFP_KERNEL);
    if (!msg_buf) return error;

    msg_len = snprintf(msg_buf, msg_len, "81%c%d%c%u", SCLDA_DELIMITER, error,
                       SCLDA_DELIMITER, fd);
    sclda_send_syscall_info(msg_buf, msg_len);
    return error;
}

int sclda_chroot(const char __user *filename) {
    struct path path;
    int error;
    unsigned int lookup_flags = LOOKUP_FOLLOW | LOOKUP_DIRECTORY;
retry:
...
SYSCALL_DEFINE2(fchmod, unsigned int, fd, umode_t, mode) {
    ...
    if (!is_sclda_allsend_fin()) return err;

    // 送信するパート
    msg_len = 200;
    msg_buf = kmalloc(msg_len, GFP_KERNEL);
    if (!msg_buf) return err;

    msg_len =
        snprintf(msg_buf, msg_len, "91%c%d%c%u%c%hu", SCLDA_DELIMITER, err,
                 SCLDA_DELIMITER, fd, SCLDA_DELIMITER, (unsigned short)mode);
    sclda_send_syscall_info(msg_buf, msg_len);
    return err;
}
...
SYSCALL_DEFINE3(fchmodat, int, dfd, const char __user *, filename, umode_t,
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 ...
                 "%c%s",
                 SCLDA_DELIMITER, retval, SCLDA_DELIMITER, dfd, SCLDA_DELIMITER,
                 (unsigned short)mode, SCLDA_DELIMITER, file_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
static inline bool setattr_vfsuid(struct iattr *attr, kuid_t kuid) {
    if (!uid_valid(kuid)) return false;
...
static inline bool setattr_vfsgid(struct iattr *attr, kgid_t kgid) {
    if (!gid_valid(kgid)) return false;
...
retry_deleg:
    ...
    if ((user != (uid_t)-1) && !setattr_vfsuid(&newattrs, uid)) return -EINVAL;
    if ((group != (gid_t)-1) && !setattr_vfsgid(&newattrs, gid)) return -EINVAL;
    inode_lock(inode);
...
SYSCALL_DEFINE5(fchownat, int, dfd, const char __user *, filename, uid_t, user,
    ...
    retval = do_fchownat(dfd, filename, user, group, flag);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len =
        snprintf(msg_buf, msg_len,
                 ...
                 "%c%d%c%s",
                 SCLDA_DELIMITER, retval, SCLDA_DELIMITER, dfd, SCLDA_DELIMITER,
                 (unsigned long)user, SCLDA_DELIMITER, (unsigned long)group,
                 SCLDA_DELIMITER, flag, SCLDA_DELIMITER, file_buf);
    sclda_send_syscall_info(msg_buf, msg_len);
...
SYSCALL_DEFINE3(lchown, const char __user *, filename, uid_t, user, gid_t,
    ...
    retval = do_fchownat(AT_FDCWD, filename, user, group, AT_SYMLINK_NOFOLLOW);
    if (!is_sclda_allsend_fin()) return retval;
...
struct file *dentry_create(const struct path *path, int flags, umode_t mode,
    ...
    error = vfs_create(mnt_user_ns(path->mnt), d_inode(path->dentry->d_parent),
                       path->dentry, mode, true);
    if (!error) error = vfs_open(path, f);
...
inline int build_open_flags(const struct open_how *how, struct open_flags *op) {
    ...
    if (WILL_CREATE(flags)) {
        if (how->mode & ~S_IALLUGO) return -EINVAL;
        op->mode = how->mode | S_IFREG;
    } else {
        if (how->mode != 0) return -EINVAL;
        op->mode = 0;
    }
    ...
    if (how->resolve & RESOLVE_NO_XDEV) lookup_flags |= LOOKUP_NO_XDEV;
    if (how->resolve & RESOLVE_NO_MAGICLINKS)
        lookup_flags |= LOOKUP_NO_MAGICLINKS;
    if (how->resolve & RESOLVE_NO_SYMLINKS) lookup_flags |= LOOKUP_NO_SYMLINKS;
    if (how->resolve & RESOLVE_BENEATH) lookup_flags |= LOOKUP_BENEATH;
    if (how->resolve & RESOLVE_IN_ROOT) lookup_flags |= LOOKUP_IN_ROOT;
    if (how->resolve & RESOLVE_CACHED) {
        /* Don't bother even trying for create/truncate/tmpfile open */
        if (flags & (O_TRUNC | O_CREAT | __O_TMPFILE)) return -EAGAIN;
...
SYSCALL_DEFINE3(open, const char __user *, filename, int, flags, umode_t,
    ...
    retval = do_sys_open(AT_FDCWD, filename, flags, mode);
...
SYSCALL_DEFINE4(openat2, int, dfd, const char __user *, filename,
    ...
    if (!(tmp.flags & O_PATH) && force_o_largefile()) tmp.flags |= O_LARGEFILE;
...
int filp_close(struct file *filp, fl_owner_t id) {
    ...
    if (filp->f_op->flush) retval = filp->f_op->flush(filp, id);
