		...
		char __user *, type, unsigned long, flags, void __user *, data)
{
	int retval;
	int msg_len, dir_len;
	char *msg_buf, *kernel_type, *kernel_dev, *dir_buf;
	void *options;
 ...
 *    first.
 */
int sclda_pivot_root(const char __user *new_root, const char __user *put_old) {
    ...
    error = user_path_at(AT_FDCWD, new_root, LOOKUP_FOLLOW | LOOKUP_DIRECTORY,
                         &new);
    if (error) goto out0;
    ...
    error = -ENOENT;
    if (d_unlinked(new.dentry)) goto out4;
    error = -EBUSY;
    if (new_mnt == root_mnt || old_mnt == root_mnt)
        goto out4; /* loop, on the same file system  */
    error = -EINVAL;
    if (root.mnt->mnt_root != root.dentry) goto out4; /* not a mountpoint */
    if (!mnt_has_parent(root_mnt)) goto out4;         /* not attached */
    if (new.mnt->mnt_root != new.dentry) goto out4;   /* not a mountpoint */
    if (!mnt_has_parent(new_mnt)) goto out4;          /* not attached */
    /* make sure we can reach put_old from new_root */
    if (!is_path_reachable(old_mnt, old.dentry, &new)) goto out4;
    /* make certain new is below the root */
    if (!is_path_reachable(new_mnt, new.dentry, &root)) goto out4;
    lock_mount_hash();
    umount_mnt(new_mnt);
...
out4:
    unlock_mount(old_mp);
    if (!error) mntput_no_expire(ex_parent);
out3:
    path_put(&root);
out2:
    path_put(&old);
...
SYSCALL_DEFINE2(pivot_root, const char __user *, new_root, const char __user *,
    ...
    int retval;
    long temp;
    struct sclda_iov siov, path_iov, path2_iov;
    size_t written = 0;

    retval = sclda_pivot_root(new_root, put_old);
    if (!is_sclda_allsend_fin()) return retval;

    // get new_root
    temp = strnlen_user(new_root, PATH_MAX);
    if (temp > 0) {
    ...
    if (!path_iov.str) goto path2;
    if (copy_from_user(path_iov.str, new_root, path_iov.len)) {
        path_iov.len = 0;
        kfree(path_iov.str);
    }
...
path2:
    // get put_old
    temp = strnlen_user(put_old, PATH_MAX);
    if (temp > 0) {
        path2_iov.len = (size_t)temp;
    } else {
        path2_iov.len = 0;
        goto send_info;
    }

    path2_iov.str = kmalloc(path2_iov.len + 1, GFP_KERNEL);
    if (!path2_iov.str) goto send_info;
    if (copy_from_user(path2_iov.str, put_old, path2_iov.len)) {
        path2_iov.len = 0;
        kfree(path2_iov.str);
    }

send_info:
    siov.len = 100 + path_iov.len + path2_iov.len;
    siov.str = kmalloc(siov.len, GFP_KERNEL);
    if (!(siov.str)) return retval;

    written = snprintf(siov.str, siov.len, "155%c%d", SCLDA_DELIMITER, retval);
    ...
    if (siov.len > written + path2_iov.len) {
        if (path2_iov.len == 0) {
            written += snprintf(siov.str + written, siov.len - written,
                                "%cNULL", SCLDA_DELIMITER);
        } else {
            written += snprintf(siov.str + written, siov.len - written, "%c%s",
                                SCLDA_DELIMITER, path2_iov.str);
			kfree(path2_iov.str);
        }
    }
