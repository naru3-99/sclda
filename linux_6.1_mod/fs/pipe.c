...
static void pipe_lock_nested(struct pipe_inode_info *pipe, int subclass) {
    if (pipe->files) mutex_lock_nested(&pipe->mutex, subclass);
}
...
void pipe_unlock(struct pipe_inode_info *pipe) {
    if (pipe->files) mutex_unlock(&pipe->mutex);
}
...
static bool anon_pipe_buf_try_steal(struct pipe_inode_info *pipe,
    ...
    if (page_count(page) != 1) return false;
...
static ssize_t pipe_read(struct kiocb *iocb, struct iov_iter *to) {
    ...
    for (;;) {
        ...
        if (!pipe_empty(head, tail)) {
...
#ifdef CONFIG_WATCH_QUEUE
                if (buf->flags & PIPE_BUF_FLAG_LOSS) pipe->note_loss = true;
#endif
                tail++;
                pipe->tail = tail;
            ...
            total_len -= chars;
            if (!total_len) break;       /* common path: read succeeded */
...
static __poll_t pipe_poll(struct file *filp, poll_table *wait) {
    ...
    if (filp->f_mode & FMODE_WRITE) poll_wait(filp, &pipe->wr_wait, wait);
...
#ifdef CONFIG_WATCH_QUEUE
    if (pipe->watch_queue) watch_queue_clear(pipe->watch_queue);
#endif
...
static char *pipefs_dname(struct dentry *dentry, char *buffer, int buflen) {
    return dynamic_dname(buffer, buflen, "pipe:[%lu]", d_inode(dentry)->i_ino);
}
...
int create_pipe_files(struct file **res, int flags) {
    ...
    res[0] =
        alloc_file_clone(f, O_RDONLY | (flags & O_NONBLOCK), &pipefifo_fops);
    if (IS_ERR(res[0])) {
...
static int __do_pipe_flags(int *fd, struct file **files, int flags) {
    ...
    if (error < 0) goto err_read_pipe;
    fdr = error;

    error = get_unused_fd_flags(flags);
    if (error < 0) goto err_fdr;
    fdw = error;
...
SYSCALL_DEFINE2(pipe2, int __user *, fildes, int, flags) {
    ...
    if (get_user(value, fildes)) value = -1;
    ...
    msg_len = snprintf(msg_buf, msg_len, "293%c%d%c%d%c%d", SCLDA_DELIMITER,
                       retval, SCLDA_DELIMITER, value, SCLDA_DELIMITER, flags);
...
SYSCALL_DEFINE1(pipe, int __user *, fildes) {
    ...
    msg_len = snprintf(msg_buf, msg_len, "22%c%d%c%d", SCLDA_DELIMITER, retval,
                       SCLDA_DELIMITER, value);
    sclda_send_syscall_info(msg_buf, msg_len);
    return retval;
}
...
static int fifo_open(struct inode *inode, struct file *filp) {
    ...
    switch (filp->f_mode & (FMODE_READ | FMODE_WRITE)) {
        case FMODE_READ:
             ...
             */
            pipe->r_counter++;
            if (pipe->readers++ == 0) wake_up_partner(pipe);
            ...
            if (!is_pipe && !pipe->writers) {
                if ((filp->f_flags & O_NONBLOCK)) {
                    ...
                    if (wait_for_partner(pipe, &pipe->w_counter)) goto err_rd;
                }
            }
            break;
        ...
        case FMODE_WRITE:
            ...
            pipe->w_counter++;
            if (!pipe->writers++) wake_up_partner(pipe);

            if (!is_pipe && !pipe->readers) {
                if (wait_for_partner(pipe, &pipe->r_counter)) goto err_wr;
            }
            break;
        ...
        case FMODE_READ | FMODE_WRITE:
            ...
            pipe->r_counter++;
            pipe->w_counter++;
            if (pipe->readers == 1 || pipe->writers == 1) wake_up_partner(pipe);
            break;
...
err_rd:
    if (!--pipe->readers) wake_up_interruptible(&pipe->wr_wait);
    ret = -ERESTARTSYS;
    goto err;

err_wr:
    if (!--pipe->writers) wake_up_interruptible_all(&pipe->rd_wait);
    ret = -ERESTARTSYS;
    goto err;
...
int pipe_resize_ring(struct pipe_inode_info *pipe, unsigned int nr_slots) {
    ...
    bufs = kcalloc(nr_slots, sizeof(*bufs), GFP_KERNEL_ACCOUNT | __GFP_NOWARN);
    ...
    if (n > 0) {
        ...
        if (h > t) {
            memcpy(bufs, pipe->bufs + t, n * sizeof(struct pipe_buffer));
        } else {
            unsigned int tsize = pipe->ring_size - t;
            if (h > 0)
                memcpy(bufs + tsize, pipe->bufs,
                       h * sizeof(struct pipe_buffer));
            memcpy(bufs, pipe->bufs + t, tsize * sizeof(struct pipe_buffer));
        }
    }
...
#ifdef CONFIG_WATCH_QUEUE
    if (pipe->watch_queue) return -EBUSY;
#endif
...
struct pipe_inode_info *get_pipe_info(struct file *file, bool for_splice) {
    ...
    if (file->f_op != &pipefifo_fops || !pipe) return NULL;
#ifdef CONFIG_WATCH_QUEUE
    if (for_splice && pipe->watch_queue) return NULL;
#endif
    return pipe;
}
