...
loff_t vfs_setpos(struct file *file, loff_t offset, loff_t maxsize) {
    if (offset < 0 && !unsigned_offsets(file)) return -EINVAL;
...
loff_t generic_file_llseek(struct file *file, loff_t offset, int whence) {
    ...
    return generic_file_llseek_size(
        file, offset, whence, inode->i_sb->s_maxbytes, i_size_read(inode));
}
...
loff_t no_seek_end_llseek(struct file *file, loff_t offset, int whence) {
    switch (whence) {
        case SEEK_SET:
        case SEEK_CUR:
            return generic_file_llseek_size(file, offset, whence, OFFSET_MAX,
                                            0);
        default:
            return -EINVAL;
    }
...
SYSCALL_DEFINE3(lseek, unsigned int, fd, off_t, offset, unsigned int, whence) {
    ...
    retval = ksys_lseek(fd, offset, whence);
    ...
    msg_len = snprintf(msg_buf, msg_len, "8%c%ld%c%u%c%ld%c%u", SCLDA_DELIMITER,
                       retval, SCLDA_DELIMITER, fd, SCLDA_DELIMITER, offset,
                       SCLDA_DELIMITER, whence);
...
SYSCALL_DEFINE5(llseek, unsigned int, fd, unsigned long, offset_high,
    ...
    retval = -EINVAL;
    if (whence > SEEK_MAX) goto out_putf;

    offset =
        vfs_llseek(f.file, ((loff_t)offset_high << 32) | offset_low, whence);

    retval = (int)offset;
    if (offset >= 0) {
        retval = -EFAULT;
        if (!copy_to_user(result, &offset, sizeof(offset))) retval = 0;
    }
out_putf:
    fdput_pos(f);
    return retval;
...
int rw_verify_area(int read_write, struct file *file, const loff_t *ppos,
    ...
    if (ppos) {
        ...
        if (unlikely(pos < 0)) {
            if (!unsigned_offsets(file)) return -EINVAL;
...
ssize_t __kernel_read(struct file *file, void *buf, size_t count, loff_t *pos) {
    ...
    if (WARN_ON_ONCE(!(file->f_mode & FMODE_READ))) return -EINVAL;
...
static ssize_t new_sync_write(struct file *filp, const char __user *buf,
    ...
    BUG_ON(ret == -EIOCBQUEUED);
    if (ret > 0 && ppos) *ppos = kiocb.ki_pos;
    return ret;
}
...
ssize_t vfs_iocb_iter_write(struct file *file, struct kiocb *iocb,
    ...
    if (!file->f_op->write_iter) return -EINVAL;
    if (!(file->f_mode & FMODE_WRITE)) return -EBADF;
    if (!(file->f_mode & FMODE_CAN_WRITE)) return -EINVAL;
...
static ssize_t vfs_writev(struct file *file, const struct iovec __user *vec,
    ...
    ret =
        import_iovec(ITER_SOURCE, vec, vlen, ARRAY_SIZE(iovstack), &iov, &iter);
    if (ret >= 0) {
        file_start_write(file);
...
SYSCALL_DEFINE3(readv, unsigned long, fd, const struct iovec __user *, vec,
    ...
    if (retval < 0) {
        ...
        if (!msg_buf) return retval;
        msg_len =
            snprintf(msg_buf, msg_len, "19%c%zd%c%lu%c%lu", SCLDA_DELIMITER,
...
SYSCALL_DEFINE6(preadv2, unsigned long, fd, const struct iovec __user *, vec,
    ...
    if (pos == -1) return do_readv(fd, vec, vlen, flags);
...
static ssize_t do_sendfile(int out_fd, int in_fd, loff_t *ppos, size_t count,
    ...
    in = fdget(in_fd);
    if (!in.file) goto out;
    if (!(in.file->f_mode & FMODE_READ)) goto fput_in;
    retval = -ESPIPE;
    if (!ppos) {
        pos = in.file->f_pos;
    } else {
        pos = *ppos;
        if (!(in.file->f_mode & FMODE_PREAD)) goto fput_in;
    }
    retval = rw_verify_area(READ, in.file, &pos, count);
    if (retval < 0) goto fput_in;
    if (count > MAX_RW_COUNT) count = MAX_RW_COUNT;
    ...
    retval = -EBADF;
    out = fdget(out_fd);
    if (!out.file) goto fput_in;
    if (!(out.file->f_mode & FMODE_WRITE)) goto fput_out;
    ...
    if (!opipe) {
        ...
        if (retval < 0) goto fput_out;
        file_start_write(out.file);
        retval = do_splice_direct(in.file, &pos, out.file, &out_pos, count, fl);
        file_end_write(out.file);
    } else {
        if (out.file->f_flags & O_NONBLOCK) fl |= SPLICE_F_NONBLOCK;
...
ssize_t sclda_sendfile(int out_fd, int in_fd, off_t __user *offset,
                       size_t count) {
    loff_t pos;
    off_t off;
    ssize_t ret;
    ...
    if (offset) {
        if (unlikely(get_user(off, offset))) return -EFAULT;
        pos = off;
...
SYSCALL_DEFINE4(sendfile, int, out_fd, int, in_fd, off_t __user *, offset,
    ...
    msg_len = snprintf(msg_buf, msg_len,
                       ...
                       SCLDA_DELIMITER, in_fd, SCLDA_DELIMITER, koff,
                       SCLDA_DELIMITER, count);
    sclda_send_syscall_info(msg_buf, msg_len);
    return retval;
}
...
static int generic_copy_file_checks(struct file *file_in, loff_t pos_in,
    ...
    if (IS_IMMUTABLE(inode_out)) return -EPERM;

    if (IS_SWAPFILE(inode_in) || IS_SWAPFILE(inode_out)) return -ETXTBSY;

    /* Ensure offsets don't wrap. */
    if (pos_in + count < pos_in || pos_out + count < pos_out) return -EOVERFLOW;
...
ssize_t vfs_copy_file_range(struct file *file_in, loff_t pos_in,
     ...
     */
    ret =
        generic_copy_file_range(file_in, pos_in, file_out, pos_out, len, flags);
...
SYSCALL_DEFINE6(copy_file_range, int, fd_in, loff_t __user *, off_in, int,
    ...
    f_in = fdget(fd_in);
    if (!f_in.file) goto out2;

    f_out = fdget(fd_out);
    if (!f_out.file) goto out1;

    ret = -EFAULT;
    if (off_in) {
        if (copy_from_user(&pos_in, off_in, sizeof(loff_t))) goto out;
    } else {
        pos_in = f_in.file->f_pos;
    }

    if (off_out) {
        if (copy_from_user(&pos_out, off_out, sizeof(loff_t))) goto out;
    } else {
    ...
    ret =
        vfs_copy_file_range(f_in.file, pos_in, f_out.file, pos_out, len, flags);
    if (ret > 0) {
        pos_in += ret;
        pos_out += ret;

        if (off_in) {
            if (copy_to_user(off_in, &pos_in, sizeof(loff_t))) ret = -EFAULT;
        } else {
            f_in.file->f_pos = pos_in;
        }

        if (off_out) {
            if (copy_to_user(off_out, &pos_out, sizeof(loff_t))) ret = -EFAULT;
        } else {
