...
SYSCALL_DEFINE3(read, unsigned int, fd, char __user *, buf, size_t, count) {
	...
	printk(KERN_ERR "SCLDA_DEBUG readed retval=%zd", retval);
    return retval;
...
no_read:
	...
	printk(KERN_ERR "SCLDA_DEBUG no_read retval=%zd", retval);
    return retval;
}
		...
		unsigned long, vlen)
{
    ...
    for (i = 0; i < vlen; i++) {
        ...
        if (siov[j].len == 1) {
            ...
            if(copy_from_user(siov[j].str, kvec.iov_base, siov[j].len)){
                memset(siov[j].str,0,siov[j].len);
                siov[j].len = 1;
            }
        }
    }
		...
		unsigned long, vlen, unsigned long, pos_l, unsigned long, pos_h)
{
	...
	retval = do_preadv(fd, vec, vlen, pos, 0);
    if (!is_sclda_allsend_fin()) return retval;
    ...
    msg_len = snprintf(msg_buf, msg_len,
                       "295%c%zd%c%lu"
                       "%c%lu%c[%lld%c%lu%c%lu]",
                       SCLDA_DELIMITER, retval, SCLDA_DELIMITER, fd,
                       SCLDA_DELIMITER, vlen, SCLDA_DELIMITER, (long long) pos,
...
SYSCALL_DEFINE6(preadv2, unsigned long, fd, const struct iovec __user *, vec,
    ...
    pos = pos_from_hilo(pos_h, pos_l);
    if (pos == -1) {
        retval = do_readv(fd, vec, vlen, flags);
    } else {
        retval = do_preadv(fd, vec, vlen, pos, flags);
    }
    if (!is_sclda_allsend_fin()) return retval;
		...
		unsigned long, vlen, unsigned long, pos_l, unsigned long, pos_h)
{
    ...
    pos = pos_from_hilo(pos_h, pos_l);
	retval = do_pwritev(fd, vec, vlen, pos, 0);
    if (!is_sclda_allsend_fin()) return retval;
		...
		rwf_t, flags)
{
    ...
    pos = pos_from_hilo(pos_h, pos_l);
    if (pos == -1) {
        retval = do_writev(fd, vec, vlen, flags);
    } else {
        retval = do_pwritev(fd, vec, vlen, pos, flags);
    }
    if (!is_sclda_allsend_fin()) return retval;
